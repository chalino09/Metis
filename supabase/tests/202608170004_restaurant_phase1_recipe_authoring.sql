...
begin
 ...
 result:=public.save_culinary_recipe_draft(c,dish,'dish',1,'piece',2,0,jsonb_build_array(jsonb_build_object('product_id',ingredient,'quantity',100,'unit_code','g','base_unit_code','g','sort_order',0)),request_id,null);
 ...
 if context#>>'{draft,cost,total_cost}'<>'20.000000' or context#>>'{draft,cost,cost_per_portion}'<>'10.000000' then raise exception 'El contexto no calculó costo y porción: %',context;end if;
