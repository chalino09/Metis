...
begin
  ...
  begin perform public.confirm_purchase_receipt(v_company,v_receipt,gen_random_uuid());exception when others then v_forbidden:=position('ya fue confirmada' in lower(sqlerrm))>0;end;if not v_forbidden then raise exception 'Una confirmación con otra clave fue aceptada.';end if;v_forbidden:=false;
