...
declare
  ...
  v_supplier uuid;v_product uuid;v_product_other uuid;v_location uuid;v_other_location uuid;v_order uuid;v_order_line uuid;v_rejected uuid;v_cancelled uuid;v_lot_order uuid;v_lot_order_line uuid;v_lot_receipt uuid;v_receipt uuid;v_receipt_two uuid;
...
begin
  ...
  v_result:=public.save_accounting_config(v_company,'MXN','2026-07-16','{"format":"4"}','{"vat_pending":"x","vat_collected":"x","vat_paid":"x","withholdings":"x"}',jsonb_build_object('adjustments',v_actor,'close',v_actor,'reopen',v_actor),'M4B recepciones',v_controls);v_cfg:=(v_result->>'id')::uuid;perform public.approve_accounting_config(v_cfg);perform public.create_accounting_period(v_company,'2026-07','2026-07-01','2026-07-31');if date_trunc('month',current_date)::date<>date '2026-07-01' then perform public.create_accounting_period(v_company,to_char(current_date,'YYYY-MM'),date_trunc('month',current_date)::date,(date_trunc('month',current_date)+interval '1 month - 1 day')::date);end if;v_result:=public.create_accounting_event_rule_set(v_company,'replacement_cost','{"purchase_receipt":"confirmation"}','M4B recepciones');v_rules:=(v_result->>'id')::uuid;
  ...
  perform public.set_product_lot_controlled(v_company,v_product,true,'Prueba de recepción por lote.','38000000-0000-4000-8000-000000000013');
  v_result:=public.save_purchase_order(v_company,null,v_supplier,'MXN','2026-07-18',null,null,null,null,0,jsonb_build_array(jsonb_build_object('product_id',v_product,'description','Controlado por lote','quantity',4,'unit_cost',12)),null);
  v_lot_order:=(v_result->>'id')::uuid;perform public.submit_purchase_order(v_company,v_lot_order,null);perform public.decide_purchase_order(v_company,v_lot_order,'approved',null);
  select id into v_lot_order_line from public.purchase_order_lines where purchase_order_id=v_lot_order;
  begin
    perform public.save_purchase_receipt(v_company,null,v_lot_order,v_location,'2026-07-18','REM-LOT-001',null,jsonb_build_array(jsonb_build_object('purchase_order_line_id',v_lot_order_line,'quantity',4)),gen_random_uuid(),null);
  exception when others then v_forbidden:=position('captura al menos un lote' in lower(sqlerrm))>0; end;
  if not v_forbidden then raise exception 'Se recibió un producto controlado sin lote.'; end if;v_forbidden:=false;
  begin
    perform public.save_purchase_receipt(v_company,null,v_lot_order,v_location,'2026-07-18','REM-LOT-001',null,jsonb_build_array(jsonb_build_object('purchase_order_line_id',v_lot_order_line,'quantity',4,'lots',jsonb_build_array(jsonb_build_object('lot_code','sin-caducidad','quantity',4)))),gen_random_uuid(),null);
  exception when others then v_forbidden:=position('caducidad' in lower(sqlerrm))>0; end;
  if not v_forbidden then raise exception 'Se recibió un lote sin caducidad.'; end if;v_forbidden:=false;
  v_result:=public.save_purchase_receipt(v_company,null,v_lot_order,v_location,'2026-07-18','REM-LOT-001',null,jsonb_build_array(jsonb_build_object('purchase_order_line_id',v_lot_order_line,'quantity',4,'lots',jsonb_build_array(jsonb_build_object('lot_code','jun-2026-a','expiration_date','2026-12-31','quantity',1),jsonb_build_object('lot_code','jun-2026-b','expiration_date','2027-01-31','quantity',3)))),gen_random_uuid(),null);
  v_lot_receipt:=(v_result->>'id')::uuid;
  if (select count(*) from public.purchase_receipt_lots lot join public.purchase_receipt_lines line on line.id=lot.purchase_receipt_line_id where line.purchase_receipt_id=v_lot_receipt)<>2 then raise exception 'No se guardaron los lotes de recepción.';end if;
  delete from public.purchase_receipt_lots where purchase_receipt_line_id in(select id from public.purchase_receipt_lines where purchase_receipt_id=v_lot_receipt);
  begin perform public.confirm_purchase_receipt(v_company,v_lot_receipt,gen_random_uuid());exception when others then v_forbidden:=position('faltan lotes' in lower(sqlerrm))>0;end;
  if not v_forbidden then raise exception 'La confirmación no revalidó los lotes.';end if;v_forbidden:=false;
  perform public.save_purchase_receipt(v_company,v_lot_receipt,v_lot_order,v_location,'2026-07-18','REM-LOT-001',null,jsonb_build_array(jsonb_build_object('purchase_order_line_id',v_lot_order_line,'quantity',4,'lots',jsonb_build_array(jsonb_build_object('lot_code','jun-2026-a','expiration_date','2026-12-31','quantity',1),jsonb_build_object('lot_code','jun-2026-b','expiration_date','2027-01-31','quantity',3)))),gen_random_uuid(),null);
  perform public.confirm_purchase_receipt(v_company,v_lot_receipt,gen_random_uuid());
  v_result:=public.get_purchase_receipt_detail(v_company,v_lot_receipt);if jsonb_array_length(v_result#>'{lines,0,lots}')<>2 then raise exception 'El detalle de recepción no expone los lotes.';end if;
  v_result:=public.get_receivable_purchase_order(v_company,v_lot_order);if jsonb_array_length(v_result#>'{lines,0,lot_suggestions}')<>2 then raise exception 'La recepción no sugiere lotes confirmados.';end if;
