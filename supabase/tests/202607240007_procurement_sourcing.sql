...
begin
  ...
  result:=public.generate_procurement_requisition_from_replenishment(c,l,'2026-08-01',null); req:=(result->>'id')::uuid;
  ...
  perform public.save_procurement_quote(c,req,s1,'MXN','2026-07-31',3,'Proveedor A cotiza producto uno',jsonb_build_array(jsonb_build_object('requisition_line_id',(select id from public.procurement_requisition_lines where requisition_id=req and product_id=p1),'available_quantity',8,'unit_price',9,'commercial_discount_percent',5,'prompt_payment_discount_percent',2)));
  perform public.save_procurement_quote(c,req,s2,'MXN','2026-07-31',5,'Proveedor B cotiza producto dos',jsonb_build_array(jsonb_build_object('requisition_line_id',(select id from public.procurement_requisition_lines where requisition_id=req and product_id=p2),'available_quantity',7,'unit_price',11,'commercial_discount_percent',0,'prompt_payment_discount_percent',1)));
  ...
  select id into ql1 from public.procurement_quote_lines where quote_id=q1; select id into ql2 from public.procurement_quote_lines where quote_id=q2;
