...
begin
  ...
  v_result:=public.save_accounting_config(v_company,'MXN','2026-07-16','{"format":"4"}','{"vat_pending":"x","vat_collected":"x","vat_paid":"x","withholdings":"x"}',jsonb_build_object('adjustments',v_actor,'close',v_actor,'reopen',v_actor),'M4B compras',v_controls);v_cfg:=(v_result->>'id')::uuid;perform public.approve_accounting_config(v_cfg);perform public.create_accounting_period(v_company,'2026-07','2026-07-01','2026-07-31');v_result:=public.create_accounting_event_rule_set(v_company,'replacement_cost','{"purchases":"confirmation"}','M4B compras');v_rules:=(v_result->>'id')::uuid;
