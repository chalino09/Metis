-- Cotizaciones y órdenes comparten una vista operativa, sin mezclar sus entidades canónicas.

create index if not exists procurement_quotes_document_catalog_idx
  on public.procurement_quotes(company_id,status,updated_at desc,id desc);
...
begin
  ...
  with documents as (
    ...
    select
      ...
      purchase_order.updated_at sort_at
  ...
  select
    ...
    coalesce(jsonb_agg(to_jsonb(paged)-'sort_at' order by sort_at desc,id desc),'[]'::jsonb)
