-- Módulo 3A: datos mínimos para activar un proveedor.
-- No modifica los proveedores ya promovidos; la regla se aplica al crear o editar.

alter table public.suppliers
  add column legal_entity_type text check (legal_entity_type in ('physical','moral'));

drop function if exists public.save_supplier(uuid,uuid,text,text,text,text,text,text,text,text,text,text,text,text,text,text,text,text,boolean,timestamptz);
...
begin
  ...
  if v_entity_type is not null and v_entity_type not in ('physical','moral') then raise exception 'El tipo de persona debe ser física o moral.';end if;
  if v_active then
    if nullif(trim(coalesce(p_legal_name,'')),'') is null then raise exception 'La razón social es obligatoria para activar el proveedor.';end if;
    if v_entity_type is null then raise exception 'El tipo de persona es obligatorio para activar el proveedor.';end if;
...
begin
 ...
 select(select total from counted),coalesce(jsonb_agg(jsonb_build_object('id',id,'code',code,'display_name',display_name,'legal_name',legal_name,'legal_entity_type',legal_entity_type,'tax_id',tax_id,'tax_regime',tax_regime,'fiscal_postal_code',fiscal_postal_code,'country_code',country_code,'contact_name',contact_name,'email',email,'phone',coalesce(phone_e164,phone),'phone_extension',phone_extension,'phone_status',phone_status,'supplier_category',supplier_category,'address_line',address_line,'neighborhood',neighborhood,'municipality',municipality,'state_name',state_name,'postal_code',postal_code,'is_active',is_active,'updated_at',updated_at) order by display_name,id),'[]'::jsonb) into v_total,v_items from paged;
...
revoke all on function public.save_supplier(uuid,uuid,text,text,text,text,text,text,text,text,text,text,text,text,text,text,text,text,text,boolean,timestamptz) from public;
grant execute on function public.save_supplier(uuid,uuid,text,text,text,text,text,text,text,text,text,text,text,text,text,text,text,text,text,boolean,timestamptz) to authenticated;
