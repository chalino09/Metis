-- En Restaurante, el nombre comercial es el único dato obligatorio del proveedor.
-- Los valores capturados conservan su validación y las identidades duplicadas se rechazan.
...
declare
  ...
  v_phone text;v_before jsonb;v_after jsonb;v_candidate uuid;v_active boolean:=coalesce(p_is_active,true);v_restaurant boolean;
begin
  ...
  select product_experience_code='restaurant' into v_restaurant from public.companies where id=p_company_id;
  if not v_restaurant and v_entity_type is null then raise exception 'El tipo de persona es obligatorio, incluso para guardar un borrador.';end if;
  ...
  if v_restaurant and v_country='' then v_country:='MX';end if;
  ...
  if v_active then
    if not v_restaurant and v_entity_type='moral' and nullif(trim(coalesce(p_legal_name,'')),'') is null then raise exception 'La razón social es obligatoria para activar una persona moral.';end if;
    if v_country='MX' and (not v_restaurant or nullif(trim(p_fiscal_postal_code),'') is not null) and trim(coalesce(p_fiscal_postal_code,'')) !~ '^[0-9]{5}$' then raise exception 'El código postal fiscal de México debe tener 5 dígitos.';end if;
    if not v_restaurant and nullif(trim(coalesce(p_email,'')),'') is null and nullif(trim(coalesce(p_phone,'')),'') is null then raise exception 'Captura un correo electrónico o teléfono para activar el proveedor.';end if;
  end if;
