-- cata_prd is the canonical product catalog, not the fiscal source.
-- Products without inline tax data are promoted with tax_category_id = NULL;
-- the existing fiscal master can enrich them later through the same staging
-- boundary. POS readiness remains the only sale gate.

begin;

alter function public.confirm_staged_import(uuid)
  rename to confirm_staged_import_before_optional_product_tax;

revoke all on function public.confirm_staged_import_before_optional_product_tax(uuid) from public, authenticated;
...
begin
  ...
  -- Inventory and the commercial imports retain the previously audited path.
  if v_batch.import_type <> 'products' then
    return public.confirm_staged_import_before_optional_product_tax(p_import_batch_id);
  end if;
  ...
  where company_id = v_batch.company_id and import_type = v_batch.import_type
    and file_sha256 = v_batch.file_sha256 and status = 'completed' and id <> v_batch.id
  limit 1;
  ...
  from public.import_files where import_batch_id = p_import_batch_id
  order by created_at limit 1;

  begin
    -- A catalog row may omit both fiscal fields. If either field is supplied,
    -- the complete pair must match an explicitly supported fiscal contract.
    if exists (
      ...
      where row_data.import_batch_id = p_import_batch_id
        ...
        and (
          nullif(row_data.normalized_data ->> 'taxCategoryCode', '') is not null
          or nullif(row_data.normalized_data ->> 'taxRate', '') is not null
        )
        and (row_data.normalized_data ->> 'taxCategoryCode', row_data.normalized_data ->> 'taxRate') not in (
          ('IVA16', '0.16'), ('IVA16', '0.160000'), ('IVA0', '0')
        )
    ) then
      raise exception 'El staging fiscal contiene una clasificación inválida o incompleta.';
    end if;

    insert into public.tax_categories(company_id, code, name, is_active)
    select distinct v_batch.company_id, row_data.normalized_data ->> 'taxCategoryCode',
    ...
    where row_data.import_batch_id = p_import_batch_id
      and row_data.detected_type = 'products'
      and coalesce((row_data.normalized_data ->> 'rejected')::boolean, false) = false
      and nullif(row_data.normalized_data ->> 'taxCategoryCode', '') is not null
    ...
    if exists (
      ...
      where category.company_id = v_batch.company_id
        and rate.jurisdiction_code = 'MX' and rate.valid_from <= now()
        and (rate.valid_to is null or rate.valid_to > now()) and rate.rate <> expected.tax_rate
    ) then
    ...
    where category.company_id = v_batch.company_id
      ...
      and not exists (
        ...
        where rate.tax_category_id = category.id and rate.jurisdiction_code = 'MX'
    ...
    select v_batch.company_id, row_data.normalized_data ->> 'alphaSku',
      nullif(row_data.normalized_data ->> 'alphaClass', ''), row_data.normalized_data ->> 'name',
      nullif(row_data.normalized_data ->> 'attribute', ''), nullif(row_data.normalized_data ->> 'unit', ''),
      nullif(row_data.normalized_data ->> 'productGroup', ''), nullif(row_data.normalized_data ->> 'subgroup', ''),
      nullif(row_data.normalized_data ->> 'productType', ''), category.id
    from public.import_staging_rows row_data
    left join public.tax_categories category
      on category.company_id = v_batch.company_id
      and category.code = nullif(row_data.normalized_data ->> 'taxCategoryCode', '')
    where row_data.import_batch_id = p_import_batch_id
      and row_data.detected_type = 'products'
      and coalesce((row_data.normalized_data ->> 'rejected')::boolean, false) = false
    on conflict(company_id, alpha_sku) do update set
      alpha_class = excluded.alpha_class, name = excluded.name, attribute = excluded.attribute,
      unit = excluded.unit, product_group = excluded.product_group, subgroup = excluded.subgroup,
    ...
    join public.products product
      on product.company_id = v_batch.company_id and product.alpha_sku = row_data.normalized_data ->> 'alphaSku'
    where row_data.import_batch_id = p_import_batch_id
      and row_data.detected_type = 'products'
      and coalesce((row_data.normalized_data ->> 'rejected')::boolean, false) = false
    on conflict(company_id, source_system, external_code) do update set
      product_id = excluded.product_id, is_primary = true, metadata = excluded.metadata, updated_at = now();

    update public.import_batches
    set status = 'completed', records_imported = v_records, completed_at = now(), closed_at = now(),
      last_activity_at = now(), notes = null
    ...
    values (v_batch.company_id, auth.uid(), 'import.completed', 'import_batch', p_import_batch_id,
      jsonb_build_object('import_type', v_batch.import_type, 'records_imported', v_records,
        ...
        'fiscal_rows', (select count(*) from public.import_staging_rows row_data
          where row_data.import_batch_id = p_import_batch_id
            and nullif(row_data.normalized_data ->> 'taxCategoryCode', '') is not null)));
...
begin
  ...
  return coalesce((
    select jsonb_agg(jsonb_build_object('tax_category_code', tax_category_code, 'total', total) order by tax_category_code)
    from (
      select row_data.normalized_data ->> 'taxCategoryCode' tax_category_code, count(*) total
      from public.import_staging_rows row_data
      where row_data.import_batch_id = p_import_batch_id
        and row_data.detected_type = 'products'
        and coalesce((row_data.normalized_data ->> 'rejected')::boolean, false) = false
...
revoke all on function public.confirm_staged_import(uuid), public.get_staged_product_tax_summary(uuid) from public;
grant execute on function public.confirm_staged_import(uuid), public.get_staged_product_tax_summary(uuid) to authenticated;
