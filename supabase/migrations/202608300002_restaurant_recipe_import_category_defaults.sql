-- Restaurante · futuras importaciones usan categorías culinarias estándar.
...
declare
  ...
  v_component_base_unit text;v_component_quantity numeric;
  ...
  for v_recipe in select * from jsonb_to_recordset(p_recipes) as x(name text,recipe_kind text,yield_quantity numeric,yield_unit_code text,components jsonb) loop
    ...
    declare v_components jsonb:='[]'::jsonb;begin
      for v_component in select * from jsonb_to_recordset(v_recipe.components) as x(canonical_key text,quantity numeric,base_unit_code text,sort_order integer,notes text) loop
        ...
        select lower(trim(product.unit)) into v_component_base_unit from public.products product where product.company_id=p_company_id and product.id=v_component_product_id and product.is_active;
        v_component_quantity:=v_component.quantity;
        if v_component_base_unit is distinct from lower(trim(v_component.base_unit_code)) then
          if lower(trim(v_component.base_unit_code))='g' and v_component_base_unit='kg' then v_component_quantity:=v_component.quantity/1000;
          elsif lower(trim(v_component.base_unit_code))='kg' and v_component_base_unit='g' then v_component_quantity:=v_component.quantity*1000;
          elsif lower(trim(v_component.base_unit_code))='ml' and v_component_base_unit='l' then v_component_quantity:=v_component.quantity/1000;
          elsif lower(trim(v_component.base_unit_code))='l' and v_component_base_unit='ml' then v_component_quantity:=v_component.quantity*1000;
          else raise exception 'La unidad de % no es compatible con la receta %.',v_component.canonical_key,v_recipe.name;
          end if;
        end if;
        v_components:=v_components||jsonb_build_array(jsonb_build_object('product_id',v_component_product_id,'quantity',v_component_quantity,'unit_code',v_component_base_unit,'base_unit_code',v_component_base_unit,'sort_order',v_component.sort_order,'notes',v_component.notes));
      end loop;
