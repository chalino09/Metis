-- Restaurante · rendimiento explícito por platillos y protección de identidad canónica.
-- Volumen esperado: altas puntuales y lotes de hasta 500 registros.
-- Las validaciones son server-side, transaccionales y no fusionan productos automáticamente.
...
begin
  ...
  if p_recipe_kind='dish' and p_portion_count<>1 then raise exception 'La receta del platillo debe representar exactamente una porción.'; end if;
  if p_recipe_kind='preparation' and p_portion_count<>trunc(p_portion_count) then raise exception 'El rendimiento por platillos debe ser un número entero.'; end if;
...
declare v_product jsonb;v_product_id uuid;v_existing jsonb;v_purchase_error text;v_normalized_name text;v_duplicate_product public.products%rowtype;
begin
  ...
  v_normalized_name:=lower(regexp_replace(trim(coalesce(p_name,'')),'\s+',' ','g'));
  if v_normalized_name='' then raise exception 'El nombre es obligatorio.'; end if;
  perform pg_advisory_xact_lock(hashtextextended(p_company_id::text||':'||v_normalized_name,0));
  select * into v_duplicate_product from public.products product
  where product.company_id=p_company_id
    and lower(regexp_replace(trim(product.name),'\s+',' ','g'))=v_normalized_name
    and (p_product_id is null or product.id<>p_product_id)
  limit 1;
  if found then raise exception 'Ya existe el registro "%". Actualiza ese registro en lugar de crear uno nuevo.',v_duplicate_product.name; end if;
  ...
  values(p_company_id,auth.uid(),'restaurant.catalog_item_saved','product',v_product_id,jsonb_build_object('request_id',p_client_request_id,'role',p_role,'reason',trim(p_reason),'canonical_name',v_normalized_name));
...
begin
  ...
  with scoped as materialized (
    select product.*,role_data.role catalog_role,price.amount price_amount,price.currency_code,purchase_unit.code purchase_unit_code,purchase.base_units_per_purchase_unit,recipe.recipe_kind,
      ...
      active_version.version_number recipe_version_number,active_version.yield_quantity recipe_yield_quantity,active_version.yield_unit_code recipe_yield_unit_code,active_version.portion_count recipe_portion_count,
      (select count(distinct parent_recipe.id)::integer from public.culinary_recipe_components component join public.culinary_recipe_versions parent_version on parent_version.id=component.recipe_version_id and parent_version.status='active' join public.culinary_recipes parent_recipe on parent_recipe.id=parent_version.recipe_id and parent_recipe.company_id=p_company_id where component.component_product_id=product.id) usage_count,
      case when active_version.id is null then 0 else (select count(*)::integer from public.culinary_recipe_components component left join public.product_culinary_roles component_role on component_role.company_id=p_company_id and component_role.product_id=component.component_product_id left join public.culinary_recipes component_recipe on component_recipe.company_id=p_company_id and component_recipe.product_id=component.component_product_id and component_recipe.recipe_kind='preparation' left join public.culinary_recipe_versions component_active on component_active.recipe_id=component_recipe.id and component_active.status='active' where component.recipe_version_id=active_version.id and (component_role.role not in ('ingredient','preparation') or component_role.role is null or (component_role.role='preparation' and component_active.id is null))) end invalid_component_count,
    ...
    left join lateral (select product_price.amount,product_price.currency_code from public.product_prices product_price where product_price.product_id=product.id and product_price.valid_from<=now() and (product_price.valid_to is null or product_price.valid_to>now()) order by product_price.valid_from desc,product_price.id desc limit 1) price on true
    where product.company_id=p_company_id and (p_is_sellable is null or (p_role='dish' and product.is_sellable=p_is_sellable) or (p_role<>'dish' and product.is_active=p_is_sellable)) and (v_query='' or lower(product.name) like '%'||v_query||'%' or lower(coalesce(product.internal_sku,'')) like '%'||v_query||'%' or lower(coalesce(product.barcode,''))=v_query or exists(select 1 from public.product_aliases alias where alias.product_id=product.id and alias.normalized_value like '%'||v_query||'%'))
  ), paged as materialized (select * from scoped order by rank,name,id limit v_size offset (v_page-1)*v_size)
  select (select count(*) from scoped),coalesce((select jsonb_agg(jsonb_build_object(
    ...
    'recipe_status',item.recipe_status,'recipe_version_number',item.recipe_version_number,'recipe_yield_quantity',item.recipe_yield_quantity,'recipe_yield_unit_code',item.recipe_yield_unit_code,'recipe_portion_count',item.recipe_portion_count,
    ...
    'blockers',case when p_role<>'dish' then '[]'::jsonb else to_jsonb(array_remove(array[case when not item.is_active then 'inactive' end,case when not item.is_sellable then 'not_sellable' end,case when item.tax_category_id is null then 'missing_tax_category' end,case when coalesce(item.price_amount,0)<=0 then 'missing_or_zero_price' end,case when item.recipe_status<>'active' then 'missing_active_recipe' end,case when item.invalid_component_count>0 then 'invalid_recipe_components' end]::text[],null)) end
...
revoke all on function public.save_culinary_recipe_draft(uuid,uuid,text,numeric,text,numeric,numeric,jsonb,uuid,uuid) from public,anon;
grant execute on function public.save_culinary_recipe_draft(uuid,uuid,text,numeric,text,numeric,numeric,jsonb,uuid,uuid),public.save_restaurant_catalog_item(uuid,uuid,text,text,text,text,text,text,boolean,boolean,uuid,text,numeric,boolean,text,timestamptz,uuid),public.search_restaurant_catalog(uuid,text,text,integer,integer,boolean) to authenticated;
