-- Add only the lis_sal documents unlocked by the audited customer identity
-- repair. This is deliberately separate from the balance repair: it never
-- closes or rewrites an existing receivable.
...
begin
  ...
  with payload as materialized (
    ...
    select source_row_hash from payload group by source_row_hash having count(*) > 1
  ), source_rows as materialized (
    select d.id,d.customer_external_code,d.source_row_hash,d.source_document_key,
      d.folio,d.document_date,d.original_amount,d.outstanding_amount,d.currency_code,
      c.status,c.promoted_customer_id
    from public.alpha_customer_migration_documents d
    join public.alpha_customer_migration_customers c
      on c.batch_id=d.batch_id and c.external_code=d.customer_external_code
    where d.batch_id=p_batch_id
  ), verified_source as materialized (
    select s.*,p.source_row_hash is not null as present_in_payload
    from source_rows s
    left join payload p on p.source_row_hash=s.source_row_hash
      and p.source_document_key=s.source_document_key
  ), payload_not_staged as materialized (
    select p.source_row_hash
    from payload p
    left join public.alpha_customer_migration_documents d
      on d.batch_id=p_batch_id and d.source_row_hash=p.source_row_hash
        and d.source_document_key=p.source_document_key
    where d.id is null
  ), eligible as materialized (
    select * from verified_source
    where present_in_payload and status='promoted' and promoted_customer_id is not null
  ), existing as materialized (
    select r.id,r.customer_id,r.source_document_key,r.source_row_hash,r.original_amount
    ...
    where e.customer_id is distinct from s.promoted_customer_id
      or e.source_row_hash is distinct from s.source_row_hash
  ...
  select jsonb_build_object(
    ...
    'can_apply',not exists(select 1 from duplicate_payload)
      and not exists(select 1 from verified_source where not present_in_payload)
      and not exists(select 1 from payload_not_staged)
    ...
    'excluded_unresolved_customer_documents',coalesce((select count(*) from verified_source where present_in_payload and (status <> 'promoted' or promoted_customer_id is null)),0),
    'excluded_unresolved_customer_amount',coalesce((select sum(outstanding_amount) from verified_source where present_in_payload and (status <> 'promoted' or promoted_customer_id is null)),0),
    'duplicate_payload_hashes',coalesce((select count(*) from duplicate_payload),0),
    'staged_documents_missing_from_source',coalesce((select count(*) from verified_source where not present_in_payload),0),
    'source_documents_not_in_staging',coalesce((select count(*) from payload_not_staged),0),
...
begin
  ...
  if not coalesce((v_preview->>'can_apply')::boolean,false) then
    raise exception 'La incorporación fue bloqueada: hay hashes, staging o documentos existentes incompatibles.';
  end if;

  with payload as materialized (
    select trim(p.customer_external_code) customer_external_code,trim(p.source_row_hash) source_row_hash,
      encode(digest(concat_ws('|',trim(p.customer_external_code),coalesce(nullif(trim(p.source_code),''),'F'),trim(p.folio),p.document_date::text,upper(trim(p.currency_code)),(round(p.original_amount,2)*100)::bigint::text),'sha256'),'hex') source_document_key
    from jsonb_to_recordset(coalesce(p_documents,'[]'::jsonb)) p(customer_external_code text,folio text,source_code text,document_date date,original_amount numeric,outstanding_amount numeric,source_row_hash text,currency_code text)
  ), eligible as materialized (
    select d.*,c.promoted_customer_id
    from public.alpha_customer_migration_documents d
    join public.alpha_customer_migration_customers c
      on c.batch_id=d.batch_id and c.external_code=d.customer_external_code
    join payload p on p.source_row_hash=d.source_row_hash and p.source_document_key=d.source_document_key
    where d.batch_id=p_batch_id and c.status='promoted' and c.promoted_customer_id is not null
  ), inserted as (
    ...
    where not exists (
      ...
      where r.company_id=v_batch.company_id and r.source_kind in ('alpha_document','alpha_opening_balance')
  ...
  set summary=summary || jsonb_build_object('receivable_backfill',jsonb_build_object(
    ...
    'total_after',v_total_after,'completed_at',now(),'completed_by',auth.uid()
  )) where id=p_batch_id;
  perform public.write_sales_audit(v_batch.company_id,'alpha_customer_migration.receivables_backfilled','alpha_customer_migration_batches',p_batch_id,jsonb_build_object(
    ...
    'amount_inserted',v_inserted_total,'total_after',v_total_after
  ));
...
revoke all on function public.preview_alpha_repaired_customer_receivable_backfill(uuid,text,jsonb),public.apply_alpha_repaired_customer_receivable_backfill(uuid,text,jsonb) from public;
grant execute on function public.preview_alpha_repaired_customer_receivable_backfill(uuid,text,jsonb),public.apply_alpha_repaired_customer_receivable_backfill(uuid,text,jsonb) to authenticated;
