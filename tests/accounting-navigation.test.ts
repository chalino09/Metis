...
test("la navegación contable es fija y no usa desplazamiento horizontal",()=>{
  ...
  assert.match(css,/\.context-nav--accounting \.context-nav__links \{[^}]*grid-template-columns:repeat\(8/);
  assert.match(css,/\.context-nav--accounting \.context-nav__links \{[^}]*overflow:visible/);
