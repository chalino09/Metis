...
test("expansión, volumen, cancelación y layout permanecen acotados",()=>{
  ...
  assert.match(ui,/for\(let step=0;step<150;step\+\+\)/);
  assert.doesNotMatch(ui,/requestAnimationFrame/);
});

test("la UI ofrece zoom, selección, filtros, detalle y tabla equivalente",()=>{
  for(const text of["Acercar","Alejar","Centrar red","Pantalla completa","Tabla accesible","Expandir 1 nivel","2 niveles","Operaciones que explican la relación"])assert.match(ui,new RegExp(text));
  for(const filter of["dateFrom","locationId","categoryId","supplierId","productId","relationType","operationalState","concentration","sizeMetric","colorMetric","edgeMetric"])assert.match(ui,new RegExp(filter));
  assert.match(ui,/bi_dependency_network_drilldown/);
  assert.match(ui,/NetworkTables/);
});
