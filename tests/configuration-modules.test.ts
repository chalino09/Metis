...
test("Configuración se organiza por módulos y conserva las rutas anteriores",()=>{
  ...
  assert.match(home,/href:"\/satrapy\/configuracion\/contabilidad"/);
  for(const moduleName of ["Empresa","Migración","Ventas","Compras","Contabilidad"])assert.match(home,new RegExp(`title:\"${moduleName}`));
});
...
test("Configuración general y Migración muestran cobertura granular",()=>{
  assert.match(shell,/label: "Configuración general"/);assert.match(home,/title="Configuración general"/);assert.match(journey,/Cobertura detectada/);assert.match(journey,/Cobertura por módulo/);assert.match(coverageSql,/total_checks/);assert.match(coverageSql,/bank_reconciliations/);
});
