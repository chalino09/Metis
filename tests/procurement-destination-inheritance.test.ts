...
test("la recepción toma y bloquea el destino de la solicitud vinculada", () => {
  ...
  assert.match(receipts, /locationId:nextCandidate\.destination_location_id\?\?current\.locationId/);
  assert.match(receipts, /Destino de recepción/);
  assert.match(receipts, /Heredado de la solicitud; las existencias se actualizarán aquí al confirmar\./);
});
