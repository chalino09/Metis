...
import { ScrollArea } from "@/app/components/reui/scroll-area"
...
function Autocomplete<Items extends readonly { items: readonly unknown[] }[]>(props: Omit<AutocompleteRoot.Props<Items[number]["items"][number]>, "items"> & { items: Items; className?: string }): React.JSX.Element
function Autocomplete<ItemValue>(props: Omit<AutocompleteRoot.Props<ItemValue>, "items"> & { items?: readonly ItemValue[]; className?: string }): React.JSX.Element
function Autocomplete({
  className,
  ...props
}: { className?: string } & Record<string, unknown>) {
  const root = <AutocompletePrimitive.Root {...props} />
  return className ? <div className={className}>{root}</div> : root
}
