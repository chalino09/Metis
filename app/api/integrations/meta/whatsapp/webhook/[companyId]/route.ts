...
async function connectionsFor(companyId:string){const admin=adminClient();const result=await admin.from("integration_connections").select("id,company_id,configuration,secret_ciphertext,status").eq("company_id",companyId).eq("provider_code","meta_whatsapp");if(result.error||!result.data?.length)throw new Error("WHATSAPP_CONNECTION_NOT_FOUND");return{admin,connections:result.data as Connection[]};}
...
export async function GET(request:NextRequest,context:Context){
  try{const {companyId}=await context.params;const {connections}=await connectionsFor(companyId);const acceptedTokens=new Set(connections.map(connection=>secretsFor(connection).verify_token).filter(Boolean));const globalToken=process.env.META_WEBHOOK_VERIFY_TOKEN?.trim();if(globalToken)acceptedTokens.add(globalToken);const mode=request.nextUrl.searchParams.get("hub.mode");const token=request.nextUrl.searchParams.get("hub.verify_token");const challenge=request.nextUrl.searchParams.get("hub.challenge");if(mode!=="subscribe"||!challenge||!token||!acceptedTokens.has(token))return new NextResponse("Forbidden",{status:403});return new NextResponse(challenge,{status:200,headers:{"content-type":"text/plain; charset=utf-8","cache-control":"no-store"}});}catch{return new NextResponse("Not found",{status:404});}
}
...
export async function POST(request:NextRequest,context:Context){
  try{
    ...
    const {admin,connections}=await connectionsFor(companyId);const appSecret=process.env.META_APP_SECRET?.trim()||secretsFor(connections[0]).app_secret;const signature=request.headers.get("x-hub-signature-256")??"";const expected=`sha256=${createHmac("sha256",appSecret).update(raw).digest("hex")}`;
    ...
    const payload=JSON.parse(raw) as MetaPayload;
    const jobs:Array<Promise<unknown>>=[];
    for(const entry of payload.entry??[])for(const change of entry.changes??[]){if(change.field!=="messages")continue;const incomingPhone=String(change.value?.metadata?.phone_number_id??"");const connection=connections.find(candidate=>String(candidate.configuration.phone_number_id??"")===incomingPhone);if(!connection)continue;const locationId=String(connection.configuration.location_id??"");if(!locationId)throw new Error("WHATSAPP_LOCATION_NOT_CONFIGURED");for(const message of change.value?.messages??[]){if(message.type!=="text"||!message.id||!message.from||!message.text?.body)continue;jobs.push(processWhatsappText(admin,{companyId,connectionId:connection.id,locationId,messageId:message.id,sender:message.from,message:message.text.body,rawPayload:raw}));}}
