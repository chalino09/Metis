...
type Body={company_id?:string;provider_code?:"meta_whatsapp"|"xai";display_name?:string;configuration?:Record<string,string>;secrets?:Record<string,string>;managed_by?:"direct"|"nango";nango_connection_id?:string|null};
...
export async function POST(request:NextRequest){
  try{
    ...
    if(managedBy==="nango"&&!nangoConnectionId)return response("Captura el Connection ID de Nango.",400);
    if(managedBy==="direct"&&provider==="xai"&&!secrets.api_key)return response("Captura la clave de API de xAI.",400);
    if(managedBy==="direct"&&provider==="meta_whatsapp"&&(!secrets.access_token||!secrets.app_secret||!secrets.verify_token))return response("Captura el token de acceso, App secret y token de verificación.",400);
    ...
    const saved=await admin.rpc("complete_integration_connection",{p_company_id:companyId,p_actor_id:auth.user.id,p_provider_code:provider,p_display_name:displayName,p_auth_mode:managedBy==="nango"?"managed":"api_key",p_configuration:configuration,p_secret_ciphertext:encryptIntegrationSecret(protectedReference,integrationEncryptionKey()),p_nango_connection_id:managedBy==="nango"?nangoConnectionId:null});
