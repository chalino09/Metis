...
type OrderDetail = {
  ...
  sale_id: string | null; completed_at: string | null; customer: Customer; location: Location; lines: OrderLine[]; payments: OrderPayment[];
};
...
export function SalesOrdersView({ companyId, permissions }: { companyId: string; permissions: string[] }) {
  ...
  async function openDetail(id: string) {
    ...
    if (detailError || !data) toast({ title: "No se abrió la orden", description: detailError?.message, tone: "error" }); else setDetail(data as OrderDetail);
  }
  ...
  async function recordPayment(event: FormEvent) {
    ...
    const fingerprint = JSON.stringify({ orderId: detail.id, paymentMethodId, amount: Number(paymentAmount), reference: paymentReference });
    setBusy(true);
    const { data, error: paymentError } = await getSupabaseClient().rpc("record_sales_deposit_order_payment", {
      p_company_id: companyId, p_order_id: detail.id, p_payment_method_id: paymentMethodId, p_amount: Number(paymentAmount),
  ...
  return <div className="content-frame sales-orders">
    <div className="page-heading"><div><span className="eyebrow">Ciclo posterior al POS</span><h1>Órdenes de venta</h1><p>Consulta pagos, preparación, entrega y documentos de operaciones iniciadas en POS o cotizaciones.</p></div><Link className="ui-button ui-button--primary ui-button--md" href="/satrapy/ventas/pos"><ShoppingCart size={16} /> Iniciar en POS</Link></div>
    <DataToolbar search={query} onSearchChange={(value) => { setQuery(value); setPage(1); }} placeholder="Buscar folio o cliente" results={total} activeFilters={status === "all" ? 0 : 1} onClear={() => { setStatus("all"); setPage(1); }} filters={<Select ariaLabel="Filtrar órdenes por entrega" value={status} onValueChange={(value) => { setStatus(value); setPage(1); }} options={[{ value: "all", label: "Todas las órdenes" }, { value: "open", label: "Pendientes de entrega" }, { value: "completed", label: "Entregadas" }]} />} />
    <DataRefreshStatus loading={loading} hasData={rows.length} />
    <DataState loading={loading && !rows.length} error={error} hasData={rows.length} emptyTitle="Aún no hay órdenes de venta." empty="Inicia una entrega posterior en POS o convierte una cotización aceptada.">
      <Table><thead><tr><th>Orden</th><th>Cliente</th><th>Entrega esperada</th><th>Pago</th><th>Entrega</th><th className="number-cell">Saldo</th></tr></thead><tbody>{rows.map((order) => <InteractiveTableRow key={order.id} label={"Abrir orden " + order.folio} onActivate={() => void openDetail(order.id)}><td><strong className="mono">{order.folio}</strong><small>{order.location_name}</small></td><td>{order.customer_name}</td><td>{order.expected_delivery_date ? new Date(order.expected_delivery_date + "T12:00:00").toLocaleDateString("es-MX") : "Por acordar"}</td><td><Badge tone={order.outstanding_amount <= 0 ? "success" : order.paid_amount > 0 ? "info" : "neutral"}>{paymentLabel(order)}</Badge><small>{money(order.paid_amount, order.currency_code)} de {money(order.total_amount, order.currency_code)}</small></td><td><Badge tone={order.status === "completed" ? "success" : "warning"}>{order.status === "completed" ? "Entregada" : "Pendiente"}</Badge></td><td className="number-cell"><strong>{money(order.outstanding_amount, order.currency_code)}</strong></td></InteractiveTableRow>)}</tbody></Table>
      <DataPagination page={page} total={total} pageSize={PAGE_SIZE} onChange={setPage} />
    </DataState>

    <Drawer open={Boolean(detail)} onOpenChange={(open) => !open && !busy && setDetail(null)} title={detail?.folio ?? "Orden de venta"} className="sales-quote-detail">{detail && <div className="sales-order-detail">
