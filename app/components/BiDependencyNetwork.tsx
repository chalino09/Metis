...
import { BiButton as Button, BiInput as Input, BiSelect as Select } from "@/app/components/reui/bi-controls";
...
export function BiDependencyNetwork({companyId}:{companyId:string}){
  ...
  return <section className="content-frame module-page bi-network bi-supplier-overview">
    <PageHeading eyebrow="Business Intelligence · Abastecimiento" title="Dependencia por proveedor"
      ...
      action={can("manage_own_bi_views")?<Button variant="secondary" onClick={()=>void save()} disabled={saving}>{saving?<LoaderCircle aria-hidden="true" className="spin" size={14}/>:<Save aria-hidden="true" size={14}/>}Guardar vista</Button>:undefined}/>
    ...
    <section className={`bi-network__filterbar bi-executive-filterbar${dirty?" has-pending":""}`} aria-label="Filtros de dependencia por proveedor">
      <div className="bi-executive-filterbar__primary">
        ...
        <Button className="bi-executive-filterbar__more" variant="secondary" size="sm" aria-expanded={advancedFiltersOpen} aria-controls="bi-supplier-advanced-filters" onClick={()=>setAdvancedFiltersOpen(current=>!current)}><Plus aria-hidden="true" size={14}/>Más filtros{advancedFilterCount?` · ${advancedFilterCount}`:""}</Button>
      ...
      {advancedFiltersOpen&&<div id="bi-supplier-advanced-filters" className="bi-executive-filterbar__advanced bi-network__advanced">
        <header><div><strong>Alcance del análisis</strong><span>Acota proveedores, productos y categorías canónicas antes de comparar dependencia.</span></div><button type="button" aria-label="Cerrar filtros" onClick={()=>setAdvancedFiltersOpen(false)}><X aria-hidden="true" size={15}/></button></header>
      ...
      </div><div className="bi-executive-filterbar__chips" aria-label="Filtros aplicados">{filterChips.slice(0,5).map(chip=><button type="button" key={chip.key} onClick={()=>removeAppliedFilter(chip.key,chip.empty)}>{chip.label}<X aria-hidden="true" size={12}/></button>)}{filterChips.length>5&&<span>+{filterChips.length-5} filtros</span>}{!filterChips.length&&<span>Sin filtros adicionales</span>}</div></div>
    </section>

    {error&&<div className="data-state data-state--error" role="alert"><strong>No se pudo consultar la dependencia por proveedor.</strong><span>{error}</span><Button size="sm" variant="secondary" onClick={()=>void load(applied,result?.pagination.page??1)}>Reintentar</Button></div>}
    {loading&&!result?<div className="data-state data-state--loading" role="status" aria-live="polite" aria-busy="true"><LoaderCircle className="spin" size={20} aria-hidden="true"/><span>Calculando dependencias completas por proveedor…</span></div>:result?<section className="bi-supplier-overview__surface" aria-busy={loading||undefined}>
      <header className="bi-supplier-overview__intro"><div><small>Mapa ejecutivo de dependencia</small><h2>Qué proveedor concentra el riesgo</h2><p>Ordenado por productos con proveedor único, dependencia alta y después importe comprobado.</p></div>{loading&&<span role="status" aria-live="polite"><LoaderCircle className="spin" size={14} aria-hidden="true"/>Actualizando</span>}</header>
...
function Metric({icon,label,value,detail}:{icon:React.ReactNode;label:string;value:string;detail:string}){return <article><span><span aria-hidden="true">{icon}</span>{label}</span><strong>{value}</strong><small>{detail}</small></article>;}
function SupplierCard({supplier,rank,currency,active,onClick}:{supplier:SupplierOverviewItem;rank:number;currency:string|null;active:boolean;onClick:()=>void}){return <button type="button" className={`bi-supplier-overview__supplier${active?" is-active":""}`} onClick={onClick} aria-pressed={active}>
  <header><span>#{rank}{supplier.code?` · ${supplier.code}`:""}</span><strong>{supplier.label}</strong><em>{money(supplier.total_amount,currency)}</em></header><div className="bi-supplier-overview__supplier-metrics"><span><PackageSearch aria-hidden="true" size={13}/>{supplier.product_count} productos</span><span><Warehouse aria-hidden="true" size={13}/>{supplier.location_count} ubicaciones</span><span><Layers3 aria-hidden="true" size={13}/>{percentage(supplier.max_concentration)} máximo</span></div>
...
function NodeFilter({companyId,type,label,value,selectedLabel,onChange}:{companyId:string;type:NodeType;label:string;value:string;selectedLabel:string;onChange:(id:string,label?:string)=>void}){
  const optionsId=useId();const [text,setText]=useState(selectedLabel),[options,setOptions]=useState<Array<{id:string;label:string;secondary?:string}>>([]),[loading,setLoading]=useState(false),[searched,setSearched]=useState(false);
  const canSearch=!value&&text.trim().length>=2;
  useEffect(()=>{if(!canSearch)return;let cancelled=false;const timer=setTimeout(()=>{setLoading(true);void getSupabaseClient().rpc("bi_search_dependency_nodes",{p_company_id:companyId,p_query:text,p_node_type:type,p_page:1,p_page_size:8}).then(response=>{if(!cancelled){if(!response.error)setOptions(((response.data as{items:typeof options})?.items)??[]);else setOptions([]);setLoading(false);setSearched(true);}});},220);return()=>{cancelled=true;clearTimeout(timer);};},[canSearch,companyId,text,type]);
  const popupOpen=canSearch&&(loading||options.length>0||searched);
  return <label className="bi-network__node-filter"><span>{label}</span><div><Input value={text} placeholder={value?"Filtro seleccionado":"Buscar…"} aria-autocomplete="list" aria-expanded={popupOpen} aria-controls={popupOpen?optionsId:undefined} onChange={event=>{setText(event.target.value);setOptions([]);setLoading(false);setSearched(false);if(!event.target.value)onChange("");}}/>{value&&<button type="button" aria-label={`Quitar ${label}`} onClick={()=>{onChange("");setText("");setOptions([]);setLoading(false);setSearched(false);}}><X aria-hidden="true" size={12}/></button>}</div>{popupOpen&&<ul id={optionsId}>{loading?<li><span role="status">Buscando…</span></li>:options.length>0?options.map(option=><li key={option.id}><button type="button" onClick={()=>{onChange(option.id,option.label);setText(option.label);setOptions([]);setSearched(false);}}><strong>{option.label}</strong><small>{option.secondary}</small></button></li>):<li><span role="status">No hay coincidencias para esta búsqueda.</span></li>}</ul>}</label>;
}
