...
import { LoadingPlaceholder } from "@/app/components/reui/loading-placeholder";
...
export function CollaboratorWeeklyScheduleModal({companyId,collaborator,open,onOpenChange,onSaved}:{companyId:string;collaborator:{id:string;name:string}|null;open:boolean;onOpenChange:(open:boolean)=>void;onSaved:(context:WeeklyScheduleContext)=>void}){
  ...
  useEffect(()=>{
    ...
    void Promise.resolve().then(async()=>{
      if(!active)return;setLoading(true);setLoadFailed(false);setError(null);setEffectiveFrom(today());setReason("");
      const {data,error:rpcError}=await getSupabaseClient().rpc("get_collaborator_weekly_schedule",{p_company_id:companyId,p_collaborator_id:collaboratorId,p_on_date:today()});
      if(!active)return;setLoading(false);
      if(rpcError){setLoadFailed(true);setError(rpcError.message.includes("schema cache")?"Falta aplicar la migración de horarios semanales.":rpcError.message);setContext(null);setDays(emptyDays());return;}
    ...
    return()=>{active=false;};
  },[collaboratorId,companyId,open,retry]);
  ...
  async function save(){
    if(!collaborator||loading||loadFailed)return;
