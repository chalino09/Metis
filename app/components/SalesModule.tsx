...
export function ReceivablesView({ companyId, permissions }: { companyId: string; permissions: string[] }) {
  ...
  return <div className="content-frame receivables-page">
    ...
    {permissions.includes("view_collection_automation") && <nav className="receivables-tabs" aria-label="Vistas de cuentas por cobrar"><Link href="/satrapy/ventas/cuentas-por-cobrar" aria-current="page">Cartera</Link><Link href="/satrapy/ventas/cuentas-por-cobrar/automatizacion">Gestiones</Link></nav>}
...
export function SalesSettingsView({ companyId, permissions, experience="core" }: { companyId: string; permissions: string[]; experience?: ProductExperience }) {
  const { toast } = useToast();
  ...
  return <div className="content-frame"><PageTitle eyebrow="Operación comercial" title="Ventas y caja" description="Configura pagos, cajas, precios, descuentos y documentos para nuevas ventas." />
  <section className="first-sale-setup" aria-labelledby="first-sale-setup-title">
    <header><div><span className="eyebrow">Inicio compartido</span><h2 id="first-sale-setup-title">Preparar primera venta</h2><p>Completa estos pasos una vez. El mismo proceso funciona para cualquier tipo de empresa.</p></div><Badge tone={firstSaleCompleted === firstSaleReady.length ? "success" : "info"}>{firstSaleCompleted} de {firstSaleReady.length}</Badge></header>
    <div className="first-sale-setup__steps">
...
function VolumeDiscountSettings({ companyId }: { companyId: string }) {
  ...
  return <section className="volume-discount-settings" aria-labelledby="volume-discount-title"><header><div><span className="eyebrow">Política automática</span><h2 id="volume-discount-title">Descuentos por volumen</h2><p>Se aplican solo a productos sin precios escalonados. Un descuento especial autorizado los sustituye.</p></div>{tiers.length > 0 && <Button variant="primary" loading={saving} disabled={loading} onClick={() => void save()}>Guardar niveles</Button>}</header>{loadError ? <div className="settings-prerequisite" role="alert"><AlertCircle size={18} aria-hidden="true" /><div><strong>No se pudo consultar la política</strong><p>{loadError}</p></div></div> : !loading && tiers.length === 0 ? <div className="volume-discount-empty"><div><strong>Sin descuentos por volumen</strong><p>Esta empresa cobra con sus precios vigentes. Crea niveles solo si forman parte de su política comercial.</p></div><Button variant="secondary" onClick={() => { setTiers(defaults); setDrafting(true); }}>Configurar niveles</Button></div> : <><div className="volume-discount-grid">{tiers.map((tier, index) => <fieldset key={tier.tier_number} disabled={loading || saving}><legend>Nivel {tier.tier_number}</legend><label>Cantidad mínima<Input type="number" min="1" step="1" inputMode="numeric" value={tier.min_quantity} onChange={(event) => updateTier(index, "min_quantity", event.target.value)} /></label><label>Cantidad máxima<Input type="number" min={tier.min_quantity} step="1" inputMode="numeric" value={tier.max_quantity ?? ""} placeholder="Sin límite" onChange={(event) => updateTier(index, "max_quantity", event.target.value)} /></label><label>Descuento %<Input type="number" min="0.01" max="99.99" step="0.01" inputMode="decimal" value={tier.discount_percent} onChange={(event) => updateTier(index, "discount_percent", event.target.value)} /></label></fieldset>)}</div><p className={`volume-discount-note${drafting ? " is-draft" : ""}`}>{drafting ? <AlertCircle size={15} aria-hidden="true" /> : <CheckCircle2 size={15} aria-hidden="true" />} {drafting ? "Borrador sin aplicar. Guarda los niveles para activarlos en esta empresa." : "Política activa. El POS recalcula el descuento al cambiar la cantidad cuando el producto no tiene precios escalonados."}</p></>}</section>;
}

type SettingsResourceKey = "payments" | "registers" | "denominations" | "prices" | "volume" | "discounts" | "ticket" | "quote";
