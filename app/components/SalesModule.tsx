...
import { Badge, Button, Drawer, Input, Modal, Select, useToast } from "@/app/components/ui/primitives";
import { DataPagination, DataRefreshStatus, DataState, DataToolbar, PageHeading as PageTitle, PagedCollection, Table } from "@/app/components/ui/data";
...
type ReceivableReceiptSummary = { id: string; folio: string; issued_at: string; amount: number | string; currency_code: string | null; payment_method: string; payment_reference: string | null };
...
type SaleRow = { sale_id: string; folio: string; location_id: string; sale_type: "cash" | "credit"; customer_name: string | null; currency_code: string; total_amount: number; completed_at: string };
...
export function PosSalesView({ companyId, companyName, cashierName, permissions }: { companyId: string; companyName: string; cashierName: string; permissions: string[] }) {
  ...
  return <div className="content-frame pos-page">
    <div className="pos-page__heading"><div><span className="eyebrow">Venta transaccional</span><h1>Punto de venta</h1><p>Los precios, impuestos, existencia y permisos se confirman en el servidor al cobrar.</p></div><Link className="pos-exit-link" href="/satrapy/ventas/historial">Salir del POS</Link></div>
    ...
    {!ownSession ? <section className="pos-start-card"><Banknote size={22} /><div><strong>Abre tu caja para iniciar.</strong><span>La apertura requiere un conteo formal por denominación. Si hay varias cajas, la selección se hace explícitamente en Caja.</span></div><Link href="/satrapy/ventas/caja" className="button button--primary">Ir a Caja</Link></section> : <>
      <div className="pos-shell">
        <section className="pos-catalog">
          <div className="pos-search"><Search size={20} /><Input ref={searchRef} value={search} onChange={(event) => setSearch(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && productTotal === 1 && products.length === 1) { event.preventDefault(); void changeItem(products[0].product_id, 1, true); } }} placeholder="Escanea o busca por palabras, SKU o código" aria-label="Buscar producto" autoFocus /><kbd>F2 cliente</kbd></div>
          <div className="pos-search-meta"><span>{productLoading ? "Buscando…" : `${products.length} de ${productTotal} resultados`}</span><span>Enter agrega coincidencia única</span></div>
        ...
        <aside className="pos-cart" id="pos-checkout">
          <div className="pos-cart__top"><div><span className="eyebrow">Carrito</span><h2>{quote?.items.length ?? 0} partidas</h2></div><span className="pos-cart__actions">{permissions.includes("apply_discount") && <Button variant="ghost" size="sm" disabled={!quote?.items.length} onClick={() => setDiscountOpen(true)}>Descuento</Button>}<Badge tone="success">Caja abierta</Badge></span></div>
          <div ref={customerPickerRef} className="pos-customer-picker"><div className="pos-customer-picker__heading"><span>Cliente</span>{permissions.includes("manage_customers") && <button type="button" onClick={() => setQuickCustomerOpen(true)}><UserPlus size={13} /> Alta rápida</button>}</div><Input ref={customerRef} role="combobox" aria-expanded={customerPickerOpen && customerResults.length > 0} aria-controls="pos-customer-options" value={customerQuery} onFocus={() => setCustomerPickerOpen(customerResults.length > 0)} onClick={() => setCustomerPickerOpen(customerResults.length > 0)} onChange={(event) => setCustomerQuery(event.target.value)} placeholder={customer ? customer.display_name : "Buscar cliente (F2)"} aria-label="Buscar cliente" />{customer && <button className="pos-customer-chip" onClick={() => void selectCustomer(null)}><span>{customer.display_name}</span><X size={14} /></button>}{customerPickerOpen && customerResults.length > 0 && <div id="pos-customer-options" className="pos-customer-results" role="listbox">{customerResults.map((item) => <button role="option" aria-selected={customer?.id === item.id} key={item.id} disabled={saleType === "credit" && (!item.credit_enabled || Boolean(item.alpha_external_code && item.migration_status !== "promoted"))} onClick={() => void selectCustomer(item)}><strong>{item.display_name}</strong><small>{item.code}{saleType === "credit" && item.available_credit !== undefined ? ` · crédito disponible ${money(item.available_credit)}` : ""}{item.alpha_external_code && item.migration_status !== "promoted" ? " · migración pendiente" : ""}</small></button>)}</div>}</div>
...
export function SalesHistoryView({ companyId, permissions }: { companyId: string; permissions: string[] }) {
  ...
  async function openTicket(saleId: string) {
    ...
    const [{ data, error: ticketError }, { data: cancellation }] = await Promise.all([
...
export function CustomersView({ companyId, permissions }: { companyId: string; permissions: string[] }) {
  ...
  useEffect(() => { const timer = window.setTimeout(() => void load(), 150); return () => window.clearTimeout(timer); }, [load]);
  return <div className="content-frame"><PageTitle eyebrow="Relación comercial" title="Clientes" description="Búsqueda server-side; los datos financieros se muestran solo a usuarios autorizados." action={permissions.includes("manage_customers") ? <Button variant="primary" onClick={() => router.push("/satrapy/ventas/clientes/nuevo")}><UserPlus size={16} /> Nuevo cliente</Button> : undefined} /><DataToolbar search={query} onSearchChange={(value) => { setQuery(value); setPage(1); }} placeholder="Buscar código, nombre, RFC o teléfono" results={total} /><DataState loading={loading} error={null} hasData={rows.length} empty="No se encontraron clientes."><div className="table-wrap surface-table"><table><thead><tr><th>Cliente</th><th>Crédito</th>{canViewCredit && <><th className="number-cell">Saldo</th><th className="number-cell">Disponible</th></>}<th aria-label="Acciones" /></tr></thead><tbody>{rows.map((row) => <tr key={row.id}><td><Link className="customer-name-link" href={`/satrapy/ventas/clientes/${row.id}`}>{row.display_name}</Link><small className="table-subline">{row.code}</small></td><td>{row.credit_enabled ? <Badge tone="success">{canViewCredit ? `${row.credit_term_days} días` : "Habilitado"}</Badge> : <Badge>Contado</Badge>}</td>{canViewCredit && <><td className="number-cell">{money(row.outstanding_amount)}</td><td className="number-cell">{row.credit_enabled ? money(row.available_credit) : "—"}</td></>}<td><Link className="settings-audit-link" href={`/satrapy/ventas/clientes/${row.id}`}>Abrir maestro <ExternalLink size={13} /></Link></td></tr>)}</tbody></table></div><SettingsPagination page={page} totalPages={Math.max(1, Math.ceil(total / 100))} onChange={setPage} /></DataState></div>;
}

export function NewCustomerMasterView({ companyId, permissions }: { companyId: string; permissions: string[] }) {
  const router = useRouter(); const { toast } = useToast(); const [name, setName] = useState(""); const [phone, setPhone] = useState(""); const [taxId, setTaxId] = useState(""); const [email, setEmail] = useState(""); const [customerType, setCustomerType] = useState(""); const [notes, setNotes] = useState(""); const [saving, setSaving] = useState(false);
  async function createCustomer(event: React.FormEvent) { event.preventDefault(); setSaving(true); const { data, error } = await getSupabaseClient().rpc("upsert_sale_customer", { p_company_id: companyId, p_customer_id: null, p_code: null, p_display_name: name, p_tax_id: taxId || null, p_email: email || null, p_phone: phone || null, p_price_list_id: null, p_credit_enabled: false, p_credit_limit: 0, p_credit_term_days: 0, p_customer_type: customerType || null, p_notes: notes || null }); setSaving(false); if (error || !data) { toast({ title: "No se pudo crear el cliente", description: rpcError(error, "Verifica los datos generales."), tone: "error" }); return; } toast({ title: "Cliente creado", description: "Quedó activo, de contado y con la lista de precios heredada.", tone: "success" }); router.replace(`/satrapy/ventas/clientes/${data}`); }
  if (!permissions.includes("manage_customers")) return <div className="content-frame"><DataState loading={false} error="No tienes permiso para crear clientes." hasData={0} empty="">{null}</DataState></div>;
  return <div className="content-frame customer-master"><Link className="customer-master__back" href="/satrapy/ventas/clientes"><ArrowLeft size={15} /> Volver a clientes</Link><header className="customer-master__heading"><div><span className="eyebrow">Clientes</span><h1>Nuevo cliente</h1><p>Registra sus datos iniciales. Al guardar podrás completar direcciones, contactos, condiciones comerciales y CxC.</p></div><Badge>Contado</Badge></header><section className="customer-master__panel"><form className="customer-master-form" onSubmit={createCustomer}><div className="customer-master-grid"><label>Nombre o razón social<Input required autoFocus value={name} onChange={(event) => setName(event.target.value)} /></label><label>Teléfono <small>Opcional, recomendado</small><Input inputMode="tel" value={phone} onChange={(event) => setPhone(event.target.value)} /></label><label>RFC <small>Opcional</small><Input value={taxId} onChange={(event) => setTaxId(event.target.value.toUpperCase())} placeholder="Se valida solo si lo capturas" /></label><label>Correo <small>Opcional</small><Input type="email" value={email} onChange={(event) => setEmail(event.target.value)} /></label><label>Tipo de cliente <small>Opcional</small><Select ariaLabel="Tipo de cliente" value={customerType} onValueChange={setCustomerType} options={[{ value: "", label: "Sin especificar" }, { value: "persona_fisica", label: "Persona física" }, { value: "persona_moral", label: "Persona moral" }]} /></label><label className="is-wide">Notas <small>Opcionales</small><Input value={notes} onChange={(event) => setNotes(event.target.value)} /></label></div><p className="settings-note">Código interno y estado activo se asignan automáticamente. El cliente queda de contado y hereda la lista de precios de la ubicación o empresa.</p><Button type="submit" variant="primary" loading={saving}>Crear cliente y continuar</Button></form></section></div>;
}

type CustomerMasterTab = "general" | "addresses" | "contacts" | "commercial" | "receivables";

export function CustomerMasterView({ companyId, customerId, permissions }: { companyId: string; customerId: string; permissions: string[] }) {
  const { toast } = useToast();
  ...
  const [receipts, setReceipts] = useState<ReceivableReceiptSummary[]>([]); const [receiptTotal, setReceiptTotal] = useState(0); const [receiptPage, setReceiptPage] = useState(1); const [receiptLoading, setReceiptLoading] = useState(false); const [receipt, setReceipt] = useState<ReceivableReceipt | null>(null);
  const [receivables, setReceivables] = useState<ReceivableDocument[]>([]); const [receivableTotal, setReceivableTotal] = useState(0); const [receivablePage, setReceivablePage] = useState(1); const [receivableQuery, setReceivableQuery] = useState(""); const [receivableDue, setReceivableDue] = useState("all"); const [receivableLoading, setReceivableLoading] = useState(false);
  ...
  const loadReceipts = useCallback(async (requestedPage = 1, append = false) => {
    setReceiptLoading(true);
    const { data, error: receiptsError } = await getSupabaseClient().rpc("list_customer_receivable_receipts", { p_company_id: companyId, p_customer_id: customerId, p_page: requestedPage, p_page_size: 25 });
    if (receiptsError) toast({ title: "No se pudieron consultar los recibos", description: rpcError(receiptsError, "Intenta nuevamente."), tone: "error" });
    else { const result = data as { items?: ReceivableReceiptSummary[]; total?: number; page?: number }; setReceipts((current) => append ? [...current, ...(result.items ?? [])] : result.items ?? []); setReceiptTotal(result.total ?? 0); setReceiptPage(result.page ?? requestedPage); }
    setReceiptLoading(false);
  }, [companyId, customerId, toast]);

  const loadReceivables = useCallback(async () => {
    setReceivableLoading(true);
    const { data, error: receivablesError } = await getSupabaseClient().rpc("list_customer_open_receivables_page", { p_company_id: companyId, p_customer_id: customerId, p_query: receivableQuery || null, p_due_status: receivableDue, p_page: receivablePage, p_page_size: 25 });
    if (receivablesError) { setReceivables([]); setReceivableTotal(0); toast({ title: "No se pudieron consultar los documentos", description: rpcError(receivablesError, "Intenta nuevamente."), tone: "error" }); }
    else { const result = data as ReceivablesPage | null; setReceivables(result?.items ?? []); setReceivableTotal(result?.pagination?.total ?? 0); }
    setReceivableLoading(false);
  }, [companyId, customerId, receivableDue, receivablePage, receivableQuery, toast]);
  ...
  useEffect(() => { if (tab !== "receivables" || !canViewCredit) return; const timer = window.setTimeout(() => { void loadReceivables(); void loadReceipts(1, false); }, 150); return () => window.clearTimeout(timer); }, [canViewCredit, loadReceipts, loadReceivables, tab]);
  ...
  async function openReceipt(receiptId: string) { const { data, error: receiptError } = await getSupabaseClient().rpc("get_customer_receivable_receipt", { p_company_id: companyId, p_receipt_id: receiptId }); if (receiptError) { toast({ title: "No se pudo abrir el recibo", description: rpcError(receiptError, "Intenta nuevamente."), tone: "error" }); return; } setReceipt(data as ReceivableReceipt); }

  if (loading && !master) return <div className="content-frame"><DataState loading error={null} hasData={0} empty="">{null}</DataState></div>;
  if (error || !master) return <div className="content-frame"><DataState loading={false} error={error ?? "Cliente no disponible."} hasData={0} empty="" errorAction={<Button onClick={() => void load()}>Reintentar</Button>}>{null}</DataState></div>;
  const tabs: Array<{ id: CustomerMasterTab; label: string }> = [{ id: "general", label: "General" }, { id: "addresses", label: "Direcciones" }, { id: "contacts", label: "Contactos" }, { id: "commercial", label: "Comercial" }, { id: "receivables", label: "CxC" }];
  return <div className="content-frame customer-master"><Link className="customer-master__back" href="/satrapy/ventas/clientes"><ArrowLeft size={15} /> Volver a clientes</Link><header className="customer-master__heading"><div><span className="eyebrow">Maestro de cliente</span><h1>{master.display_name}</h1><p>{master.code} · {master.commercial.credit_enabled ? "Crédito" : "Contado"}</p></div>{master.is_imported && <Badge tone={master.migration_status === "adjustment_pending" ? "warning" : "info"}>Datos protegidos</Badge>}</header>{master.is_imported && <div className="inline-status"><span>Los datos promovidos conservan trazabilidad y solo cambian mediante un ajuste auditado.</span>{permissions.includes("import_data") && <Button size="sm" variant="secondary" onClick={() => setAdjustmentOpen(true)}>Solicitar ajuste</Button>}</div>}<nav className="customer-master__tabs" aria-label="Secciones del cliente">{tabs.map((item) => <button className={tab === item.id ? "is-active" : ""} onClick={() => setTab(item.id)} key={item.id}>{item.label}</button>)}</nav><section className="customer-master__panel">{tab === "general" && <form className="customer-master-form" onSubmit={saveGeneral}><div className="customer-master-grid"><label>Nombre<Input value={name} onChange={(event) => setName(event.target.value)} disabled={!editable} /></label><label>RFC<Input value={taxId} onChange={(event) => setTaxId(event.target.value.toUpperCase())} disabled={!editable} /></label><label>Código interno<Input value={master.code} disabled /></label>{master.source_reference && <label>Referencia de origen<Input value={master.source_reference} disabled /></label>}</div>{editable && <Button type="submit" variant="primary" loading={saving}>Guardar General</Button>}</form>}
