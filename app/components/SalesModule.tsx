...
import { DataRefreshStatus, DataState, InteractiveTableRow, PageHeading as PageTitle, PagedCollection, Table } from "@/app/components/ui/data";
import { Alert as ReuiAlert, AlertAction as ReuiAlertAction, AlertDescription as ReuiAlertDescription, AlertTitle as ReuiAlertTitle } from "@/components/reui/alert";
...
import { Empty as ReuiEmpty, EmptyDescription as ReuiEmptyDescription, EmptyHeader as ReuiEmptyHeader, EmptyMedia as ReuiEmptyMedia, EmptyTitle as ReuiEmptyTitle } from "@/app/components/reui/empty";
import { ScrollArea as ReuiScrollArea } from "@/app/components/reui/scroll-area";
import { Tabs as ReuiTabs, TabsContent as ReuiTabsContent, TabsList as ReuiTabsList, TabsTrigger as ReuiTabsTrigger } from "@/app/components/reui/tabs";
import { SalesButton as Button, SalesCurrencyInput as CurrencyInput, SalesDataPagination as DataPagination, SalesDataToolbar as DataToolbar, SalesInput as Input, SalesSelect as Select } from "@/app/components/reui/sales-controls";
...
export function PosSalesView({ companyId, companyName, cashierName, permissions, experience="core" }: { companyId: string; companyName: string; cashierName: string; permissions: string[]; experience?: ProductExperience }) {
  ...
  // Focus the scanner field after layout without changing the user's scroll.
  useLayoutEffect(() => {
    if (!context || initialSearchFocusRef.current) return;
    const frame = window.requestAnimationFrame(() => {
      const input = searchRef.current;
      if (!input || initialSearchFocusRef.current) return;
  ...
  function scrollCartWithPointer(event: React.WheelEvent<HTMLDivElement>) {
    const viewport = event.currentTarget.querySelector<HTMLElement>('[data-slot="scroll-area-viewport"]');
    if (!viewport || viewport.scrollHeight <= viewport.clientHeight || event.deltaY === 0) return;
    ...
    if (viewport.scrollTop !== previousScrollTop) event.preventDefault();
  }
    ...
    {!ownSession ? <section className="pos-start-card"><Banknote size={22} /><div><strong>Abre tu caja para iniciar.</strong><span>La apertura requiere un conteo formal por denominación. Si hay varias cajas, la selección se hace explícitamente en Caja.</span></div><Link href="/satrapy/ventas/caja" className="button button--primary">Ir a Caja</Link></section> : <>
      <ReuiTabs value={posTab} onValueChange={(value) => setPosTab(value as "current" | "held")} className="pos-reui-tabs">
        <div className="pos-workspace-header">
          <ReuiTabsList className="pos-workspace-tabs" aria-label="Ventas del punto de venta">
      ...
      </ReuiTabsContent><ReuiTabsContent value="held" className="pos-held-workspace" id="pos-panel-held">
...
export function SalesSettingsView({ companyId, permissions, experience="core", initialResource }: { companyId: string; permissions: string[]; experience?: ProductExperience; initialResource?: SettingsResourceKey }) {
  ...
  return <div className={`content-frame sales-reui-page sales-settings-page${isWideResource ? " sales-settings-wide" : ""}`}>{isWideResource ? <>
