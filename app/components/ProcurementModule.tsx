...
export function ProcurementView({
  ...
  const openManual = useCallback(async (preselectedProductId = "") => {
    ...
    if (preselectedProductId) {
      ...
      const match = ((data as { items?: Product[] } | null)?.items ?? []).find((product) => product.id === preselectedProductId);
      if (match) {
        selectedProductId = match.id;
        const sku = match.internal_sku ?? match.alpha_sku ?? match.barcode ?? "Sin código";
        setManualProductQuery(`${match.name} · ${sku}`);
      }
    }
    setManual({ locationId: "", productId: selectedProductId, quantity: "", targetDate: "", reason: "" });
    setManualOpen(true);
  }, [companyId]);
