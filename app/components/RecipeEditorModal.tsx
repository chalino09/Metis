  ...
  return <Modal open={open} onOpenChange={value=>{if(!saving&&!activating)onOpenChange(value);}} eyebrow={recipeKind==="preparation"?"Base reutilizable":"Platillo"} title={product?.name??(recipeKind==="preparation"?"Nueva base":"Nuevo platillo")} description={recipeKind==="preparation"?"Define qué lleva una tanda y cuánto produce.":"Captura únicamente lo que consume una porción."} className="recipe-editor" closeDisabled={saving||activating} footer={footer}>
    {loading?<p className="recipe-editor__loading" role="status">Cargando receta…</p>:<div ref={bodyRef} className="recipe-editor__body">
      ...
      {step==="components"?<section className="recipe-editor__ingredients" aria-labelledby="recipe-ingredients-title">
        ...
        <div className="recipe-editor__setup-fields recipe-editor__setup-fields--primary"><Field label="Cantidad producida"><Input data-recipe-detail inputMode="decimal" aria-invalid={numeric.yield<=0||undefined} aria-describedby={numeric.yield<=0?yieldErrorId:undefined} value={yieldQuantity} onChange={event=>setYieldQuantity(event.target.value)}/>{numeric.yield<=0&&<small id={yieldErrorId} className="ui-field__error">Escribe una cantidad mayor que cero.</small>}</Field><Field label="Unidad del rendimiento"><Select value={yieldUnit} onValueChange={setYieldUnit} ariaLabel="Unidad del rendimiento" options={units}/></Field><Field label="¿Para cuántos platillos rinde?"><Input data-recipe-detail inputMode="numeric" aria-invalid={(!Number.isInteger(numeric.portions)||numeric.portions<=0)||undefined} aria-describedby={!Number.isInteger(numeric.portions)||numeric.portions<=0?`${yieldErrorId}-portions`:undefined} value={portionCount} onChange={event=>setPortionCount(event.target.value)}/><small>Usa porciones del mismo tamaño. Cada receta conservará su consumo real de la base.</small>{(!Number.isInteger(numeric.portions)||numeric.portions<=0)&&<small id={`${yieldErrorId}-portions`} className="ui-field__error">Escribe un número entero mayor que cero.</small>}</Field></div>
