...
export function RecipeEditorModal({companyId,product,recipeKind="dish",open,onOpenChange,onCreateIngredient}:{companyId:string;product:ProductRef|null;recipeKind?:"dish"|"preparation";open:boolean;onOpenChange:(open:boolean)=>void;onCreateIngredient?:()=>void}) {
  ...
  const hydrate=useCallback((next:Context)=>{
    ...
    setPortionCount(recipeKind==="dish"?"1":String(version?.portion_count??1));
  ...
  },[companyId,lines,open,product?.id,query,searchOpen]);

  const numeric=useMemo(()=>({yield:numberValue(yieldQuantity),portions:numberValue(portionCount),waste:numberValue(waste)}),[portionCount,waste,yieldQuantity]);
  ...
  const validDetails=numeric.waste>=0&&numeric.waste<100&&(recipeKind==="dish"||(numeric.yield>0&&Number.isInteger(numeric.portions)&&numeric.portions>0));
  ...
  async function saveDraft(announce=true){
    ...
    const recipeValues=recipeKind==="dish"?{yield:1,yieldUnit:"piece",portions:1}:{yield:numeric.yield,yieldUnit,portions:numeric.portions};
  ...
  return <Modal open={open} onOpenChange={value=>{if(!saving&&!activating)onOpenChange(value);}} eyebrow={recipeKind==="preparation"?"Base reutilizable":"Platillo"} title={product?.name??(recipeKind==="preparation"?"Nueva base":"Nuevo platillo")} description={recipeKind==="preparation"?"Define qué lleva una tanda y cuánto produce.":"Captura únicamente lo que consume una porción."} className="recipe-editor" closeDisabled={saving||activating} footer={footer}>
    {loading?<p className="recipe-editor__loading" role="status">Cargando receta…</p>:<div ref={bodyRef} className="recipe-editor__body">
      ...
      {step==="components"?<section className="recipe-editor__ingredients" aria-labelledby="recipe-ingredients-title">
        ...
        <header><span className="recipe-editor__step-label">Paso 2 de 2</span><h3 id="recipe-yield-title">¿Cuánto produce una tanda?</h3><p>Indica el rendimiento físico y para cuántos platillos de referencia alcanza.</p></header>
        <div className="recipe-editor__setup-fields recipe-editor__setup-fields--primary"><Field label="Cantidad producida"><Input data-recipe-detail inputMode="decimal" aria-invalid={numeric.yield<=0||undefined} aria-describedby={numeric.yield<=0?yieldErrorId:undefined} value={yieldQuantity} onChange={event=>setYieldQuantity(event.target.value)}/>{numeric.yield<=0&&<small id={yieldErrorId} className="ui-field__error">Escribe una cantidad mayor que cero.</small>}</Field><Field label="Unidad del rendimiento"><Select showAllOnOpen value={yieldUnit} onValueChange={setYieldUnit} ariaLabel="Unidad del rendimiento" options={units}/></Field><Field label="¿Para cuántos platillos rinde?"><Input data-recipe-detail inputMode="numeric" aria-invalid={(!Number.isInteger(numeric.portions)||numeric.portions<=0)||undefined} aria-describedby={!Number.isInteger(numeric.portions)||numeric.portions<=0?`${yieldErrorId}-portions`:undefined} value={portionCount} onChange={event=>setPortionCount(event.target.value)}/><small>Usa porciones del mismo tamaño. Cada receta conservará su consumo real de la base.</small>{(!Number.isInteger(numeric.portions)||numeric.portions<=0)&&<small id={`${yieldErrorId}-portions`} className="ui-field__error">Escribe un número entero mayor que cero.</small>}</Field></div>
        ...
        <section className="recipe-editor__review" aria-labelledby="recipe-review-title"><div><h4 id="recipe-review-title">Revisión de la tanda</h4><p>{lines.length} {lines.length===1?"componente":"componentes"} · {yieldQuantity||"—"} {units.find(unit=>unit.value===yieldUnit)?.label??yieldUnit} · rinde para {portionCount||"—"} {numberValue(portionCount)===1?"platillo":"platillos"}</p></div><Badge tone={valid?"success":"warning"}>{valid?"Lista para guardar":"Revisa los datos"}</Badge></section>
        {summary}
      </section>}
    </div>}
  </Modal>;
