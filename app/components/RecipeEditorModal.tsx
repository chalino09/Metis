...
export function RecipeEditorModal({companyId,product,recipeKind="dish",open,onOpenChange,onCreateIngredient}:{companyId:string;product:ProductRef|null;recipeKind?:"dish"|"preparation";open:boolean;onOpenChange:(open:boolean)=>void;onCreateIngredient?:()=>void}) {
  ...
  useEffect(()=>{
    ...
    const timer=window.setTimeout(async()=>{
      ...
      const {data,error}=await getSupabaseClient().rpc("search_restaurant_recipe_components",{p_company_id:companyId,p_query:trimmed,p_page:1,p_page_size:20});
      if(!active)return;
      setSearching(false);
  ...
  return <Modal open={open} onOpenChange={value=>{if(!saving&&!activating)onOpenChange(value);}} eyebrow={recipeKind==="preparation"?"Base reutilizable":"Platillo"} title={product?.name??(recipeKind==="preparation"?"Nueva base":"Nuevo platillo")} description={recipeKind==="preparation"?"Define qué lleva una tanda y cuánto produce.":"Captura únicamente lo que consume una porción."} className="recipe-editor" closeDisabled={saving||activating} footer={footer}>
    {loading?<p className="recipe-editor__loading" role="status">Cargando receta…</p>:<div ref={bodyRef} className="recipe-editor__body">
      ...
      {step==="components"?<section className="recipe-editor__ingredients" aria-labelledby="recipe-ingredients-title">
        ...
        <div ref={searchPickerRef} className="recipe-editor__component-search"><label htmlFor={searchId}>Buscar insumo o base</label><div className="recipe-editor__search"><Search size={17} aria-hidden="true"/><Input ref={searchInputRef} id={searchId} autoFocus value={query} onFocus={()=>setSearchOpen(true)} onChange={event=>{setQuery(event.target.value);setSearchOpen(true);setCandidates([]);setSearchError(null);setSearching(Boolean(event.target.value.trim()));}} onKeyDown={event=>{if(event.key==="Escape"){event.preventDefault();setSearchOpen(false);}}} placeholder="Ej. pollo, jitomate o salsa roja" aria-describedby={`${searchId}-hint`} aria-expanded={searchOpen} aria-controls={`${searchId}-results`} aria-haspopup="listbox"/></div><small id={`${searchId}-hint`}>Escribe para buscar un insumo o una base reutilizable.</small>{searchOpen&&<div id={`${searchId}-results`} className="recipe-editor__results" aria-label="Insumos y bases disponibles" aria-live="polite">{searching?<p role="status">Buscando…</p>:searchError?<div className="recipe-editor__no-results" role="alert"><p>{searchError}</p></div>:candidates.length?candidates.map(item=><button type="button" key={item.id} onClick={()=>add(item)}><span><strong>{item.name}</strong><small>{item.internal_sku} · {item.recipe_kind==="preparation"?"Base reutilizable":item.unit??"Sin unidad"}{item.usage_count>0?` · ${item.usage_count} ${item.usage_count===1?"receta":"recetas"}`:""}</small></span><Plus size={17} aria-hidden="true"/></button>):<div className="recipe-editor__no-results"><p>{query.trim()?"No hay un insumo o base activa con ese nombre.":"Escribe el nombre de un insumo o una base para buscar."}</p>{onCreateIngredient&&lines.length===0&&<Button type="button" size="sm" variant="secondary" onClick={onCreateIngredient}><Plus size={15} aria-hidden="true"/> Crear insumo</Button>}</div>}</div>}</div>
