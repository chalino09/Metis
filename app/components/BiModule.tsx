...
type DashboardWidget={id:string;dashboard_id:string;saved_view_id:string;widget_type:"kpi"|"chart"|"table";title?:string|null;filter_mode:"inherit"|"own";position:number;width:number;height:number;view_name:string;status:"ready"|"error";error?:string;definition?:Record<string,unknown>;result?:ExplorerResult};
...
export function BiModule({ companyId, view }: { companyId: string; view: BiView }) {
  ...
  if (view !== "summary") return <BiRoadmap view={view} />;
  return <BiExecutiveSummary companyId={companyId} />;
}

function BiRoadmap({ view }: { view: "network" }) {
  const content = {
    network: { eyebrow: "Pendiente", title: "Red de dependencias", text: "La red proveedor → producto → ubicación cargará subredes filtradas y se expandirá bajo demanda. Es una vista secundaria, no la portada.", icon: GitFork },
  }[view];
  const Icon = content.icon;
  return <section className="content-frame module-page bi-roadmap">
    <PageHeading eyebrow="Business Intelligence" title={content.title} description={content.text} />
    <article><Icon size={28} /><span className="eyebrow">{content.eyebrow}</span><h2>Base preparada, entrega incremental</h2><p>La navegación y el permiso ya forman parte del módulo. Esta sección se activará sobre el mismo catálogo de métricas y las mismas fuentes canónicas del Resumen ejecutivo.</p></article>
  </section>;
}
...
function BiWorkspace({companyId}:{companyId:string}){
  ...
  const[widgetView,setWidgetView]=useState("");const[widgetType,setWidgetType]=useState<"kpi"|"chart"|"table">("chart");
  ...
  return <section className="content-frame module-page bi-module bi-workspace">
    ...
    <Modal open={dialog==="widget"}onOpenChange={open=>!open&&setDialog(null)}eyebrow="Widget" title="Agregar desde una vista"description="El widget vuelve a consultar las fuentes canónicas."footer={<><Button variant="secondary"onClick={()=>setDialog(null)}>Cancelar</Button><Button disabled={!widgetView}onClick={()=>void addWidget()}>Agregar</Button></>}><div className="bi-save-form"><label><span>Vista guardada</span><Select ariaLabel="Vista guardada"value={widgetView||"none"}onValueChange={setWidgetView}options={[{value:"none",label:"Selecciona una vista"},...views.filter(v=>v.availability.available).map(v=>({value:v.id,label:v.name}))]}/></label><label><span>Tipo</span><Select ariaLabel="Tipo de widget"value={widgetType}onValueChange={v=>setWidgetType(v as typeof widgetType)}options={[{value:"kpi",label:"KPI"},{value:"chart",label:"Gráfica"},{value:"table",label:"Tabla"}]}/></label></div></Modal>
  </section>;
}
