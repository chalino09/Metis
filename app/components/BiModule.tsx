...
function BiAlertsModule({companyId}:{companyId:string}){
  ...
  return<section className="content-frame module-page bi-module bi-alerts-page">
    ...
    <div className="bi-alerts-filters" aria-label="Filtros de alertas">
      <label><span>Estado</span><Select showAllOnOpen ariaLabel="Estado de alerta" value={status} onValueChange={value=>{setStatus(value);setPage(1);}} options={[{value:"active",label:"Activas"},{value:"reviewed",label:"Revisadas"},{value:"resolved",label:"Resueltas"},{value:"all",label:"Todas"}]}/></label>
      <label><span>Severidad</span><Select showAllOnOpen ariaLabel="Severidad de alerta" value={severity} onValueChange={value=>{setSeverity(value);setPage(1);}} options={[{value:"all",label:"Todas"},{value:"critical",label:"Crítica"},{value:"warning",label:"Advertencia"},{value:"informational",label:"Informativa"}]}/></label>
      <label><span>Métrica</span><Select showAllOnOpen ariaLabel="Métrica de alerta" value={metric} onValueChange={value=>{setMetric(value);setPage(1);}} options={[{value:"all",label:"Todas"},{value:"net_sales",label:"Ventas netas"},{value:"gross_margin",label:"Margen bruto"}]}/></label>
      <label><span>Ubicación</span><Select showAllOnOpen ariaLabel="Ubicación de alerta" value={location} onValueChange={value=>{setLocation(value);setPage(1);}} options={[{value:"all",label:"Todas"},...accessibleLocations.map(item=>({value:item.id,label:item.name}))]}/></label>
    </div>
...
function BiAlertDetail({alert,companyId,canManage,onClose,onCorrected,onReview,onResolve}:{alert:BiOperationalAlert|null;companyId:string;canManage:boolean;onClose:()=>void;onCorrected:()=>Promise<void>;onReview:(alert:BiOperationalAlert)=>Promise<void>;onResolve:(alert:BiOperationalAlert)=>void}){
  ...
  if(!alert)return null;const visual=alertSeverity(alert);const context=alertInvestigationContext(alert);const isRestaurantMargin=alert.rule_code==="restaurant_dish_margin_below_threshold";const observedLabel=alert.observed_value==null?"No disponible":isRestaurantMargin?`${alert.observed_value.toLocaleString("es-MX",{maximumFractionDigits:2})}%`:formatMoney(alert.observed_value);const thresholdLabel=alert.comparison_value!=null?(isRestaurantMargin?`${alert.comparison_value.toLocaleString("es-MX",{maximumFractionDigits:2})}%`:formatMoney(alert.comparison_value)):alert.threshold_value!=null?`${alert.threshold_value.toLocaleString("es-MX",{maximumFractionDigits:2})}${isRestaurantMargin?"%":""}`:"Cobertura completa requerida";
    ...
    {catalogError?<div className="bi-partial-state" role="alert"><AlertCircle size={16}/><span><strong>Catálogo no disponible</strong>{catalogError}</span></div>:!catalog?<div className="bi-refreshing"><LoaderCircle className="spin" size={15}/>Cargando catálogo de compatibilidad…</div>:<>
      <div className="bi-explorer-builder">
        ...
        <section className="bi-explorer-config"><span className="eyebrow">2 · Forma del análisis</span>
          <label><span>Dimensión compatible</span><Select showAllOnOpen ariaLabel="Dimensión compatible" value={effectiveDimension} onValueChange={setDimension} options={compatibleDimensions.map(code=>({value:code,label:catalog.dimensions.find(item=>item.code===code)?.name??code}))}/></label>
          <label><span>Visualización apropiada</span><Select showAllOnOpen ariaLabel="Visualización apropiada" value={effectiveVisualization} onValueChange={value=>setVisualization(value as typeof visualization)} options={compatibleVisualizations.map(code=>({value:code,label:EXPLORER_VIZ[code]}))}/></label>
  ...
  return <section className="content-frame module-page bi-module">
    ...
    <BiFilterBar className="bi-executive-filterbar" pending={dirty} ariaLabel="Filtros del Resumen ejecutivo">
      <div className="bi-executive-filterbar__primary">
        ...
        <label><span>Comparar con</span><Select showAllOnOpen ariaLabel="Periodo de comparación" value={filters.comparisonMode} onValueChange={value=>setFilters(current=>({...current,comparisonMode:value as BiFilters["comparisonMode"]}))} options={COMPARISON_OPTIONS}/></label>
