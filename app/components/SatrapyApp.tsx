...
import { Badge, Button, Field, Input, Modal, Select, ToastProvider, useToast } from "@/app/components/ui/primitives";
import { Button as InventoryButton, Input as InventoryInput, Select as InventorySelect, Drawer as InventoryDrawer, Modal as InventoryModal, DataToolbar as InventoryDataToolbar } from "./reui/inventory-controls";
...
function RouteModuleLoading() {
  return <LoadingPlaceholder kind="page" label="Cargando módulo…" />;
}
...
export function SatrapyRouteContent() {
  ...
  if (activeView === "inventory_transfers") return <InventoryTransfersView key={appState.membership.companyId} companyId={appState.membership.companyId} permissions={appState.membership.permissions} />;
  if (activeView === "inventory_replenishment") return <InventoryReplenishmentView key={appState.membership.companyId} companyId={appState.membership.companyId} permissions={appState.membership.permissions} />;
...
function LoadingScreen() {
  return <main className="app-shell app-loading-shell app-shell--inventory" aria-busy="true">
...
function InventoryView({ companyId, permissions }: { companyId: string; permissions: string[] }) {
  ...
  return <div className="content-frame inventory-product-inquiry"><PageHeading eyebrow="Existencia operativa" title="Inventario por ubicación" description={directLocationView ? "Consulta la existencia y los movimientos de la sucursal seleccionada." : "Consulta cada producto con su existencia total y despliega el saldo de cada sucursal."} action={<div className="inventory-heading-actions">{canInitializeInventory&&<InventoryButton variant="primary" onClick={openInventoryOpening}><Plus size={16}/> Registrar inventario inicial</InventoryButton>}<InventoryButton variant="secondary" onClick={refresh}><RefreshCw size={16} /> Actualizar</InventoryButton></div>} /><InventoryDataToolbar search={search} onSearchChange={changeSearch} placeholder="Buscar producto o SKU" filters={<InventorySelect value={selectedLocation} onValueChange={changeLocation} ariaLabel="Filtrar por ubicación" options={locationOptions} />} activeFilters={(search.trim() ? 1 : 0) + (selectedLocation !== "all" ? 1 : 0)} onClear={clearFilters} results={total} /><DataState preserveData loading={loading} error={error} errorAction={<InventoryButton variant="secondary" size="sm" onClick={refresh}>Reintentar</InventoryButton>} hasData={rows.length} empty={empty}><div className="table-wrap surface-table inventory-product-table"><table><thead><tr><th>SKU</th><th>Producto</th><th className="number-cell">{directLocationView ? "Existencia" : "Existencia total"}</th><th>{directLocationView ? "Movimiento reciente" : "Sucursales"}</th><th>Actualización</th><th aria-label="Acciones" /></tr></thead><tbody>{rows.map((product) => {
    ...
    <InventoryDrawer open={openingOpen} onOpenChange={open=>{if(!open)closeInventoryOpening();}} title="Registrar inventario inicial" className="inventory-opening-drawer"><form className="inventory-opening" onSubmit={saveInventoryOpening}>
      ...
      <Field label="Sucursal"><InventorySelect value={openingLocation} onValueChange={value=>{setOpeningLocation(value);setOpeningPage(1);setOpeningQuantities({});}} ariaLabel="Sucursal para inventario inicial" options={accessibleLocations.map(location=>({value:location.id,label:`${location.external_code} · ${location.name}`}))}/></Field>
      {!openingEligible?<div className="inventory-opening__blocked" role="status"><strong>Esta sucursal ya comenzó a operar</strong><p>Para cambiar sus cantidades usa una recepción de compra o un conteo físico; el inventario inicial no se puede repetir.</p></div>:<>
        ...
        <InventoryDataToolbar search={openingQuery} onSearchChange={setOpeningQuery} placeholder="Buscar producto o código" results={openingTotal}/>
        <DataState loading={openingLoading} error={openingError} errorAction={<InventoryButton size="sm" variant="secondary" onClick={()=>void loadOpeningProducts()}>Reintentar</InventoryButton>} hasData={openingRows.length} empty="No hay productos activos que controlen existencias."><div className="table-wrap inventory-opening__table"><table><thead><tr><th>Producto</th><th>Unidad</th><th className="number-cell">Cantidad inicial</th></tr></thead><tbody>{openingRows.map(product=><tr key={product.product_id}><td><strong>{product.name}</strong><small className="mono">{product.product_code}</small></td><td>{product.unit??"—"}</td><td className="number-cell"><InventoryInput inputMode="decimal" aria-label={`Cantidad inicial de ${product.name}`} value={openingQuantities[product.product_id]??""} onChange={event=>setOpeningQuantities(current=>({...current,[product.product_id]:event.target.value.replace(/[^\d.,]/g,"")}))} placeholder="0"/></td></tr>)}</tbody></table></div></DataState>
...
function InventoryReplenishmentView({ companyId, permissions }: { companyId: string; permissions: string[] }) {
  ...
  return <div className="content-frame inventory-replenishment"><PageHeading eyebrow="Planeación de inventario" title="Reabastecimiento" description="Consulta faltantes por ubicación y prepara una solicitud de compra cuando corresponda. No crea órdenes automáticamente." action={<>{canManage && <InventoryButton variant="secondary" onClick={() => setPolicyEditorOpen(true)}>Configurar mínimos y máximos</InventoryButton>}{canPrepareRequisition && <InventoryButton variant="secondary" disabled={locationFilter === "all" || statusCounts.unattended === 0} loading={busy} onClick={() => setRequisitionConfirmationOpen(true)}>Crear solicitud</InventoryButton>}<InventoryButton variant="secondary" loading={loading} onClick={() => void load()}><RefreshCw size={16} /> Actualizar</InventoryButton></>} />
    {canManage && <InventoryDrawer open={policyEditorOpen} onOpenChange={(open) => { if (!busy) setPolicyEditorOpen(open); }} title="Configurar mínimos y máximos" className="inventory-replenishment-drawer"><div className="inventory-replenishment-policy-editor"><p className="settings-note">Define mínimos y máximos por ubicación. Esta configuración no mueve inventario ni crea órdenes de compra.</p><section className="inventory-transfer-builder inventory-replenishment-builder"><header><div><span className="eyebrow">Políticas de inventario</span><h2>Selecciona los productos</h2></div><InventoryButton variant="secondary" size="sm" disabled={!policyLocationId} onClick={() => setBulkImportOpen(true)}>Importar políticas</InventoryButton></header>
      <div className="inventory-transfer-route"><label>Ubicación<InventorySelect ariaLabel="Ubicación para configurar políticas" value={policyLocationId || "unselected"} onValueChange={selectPolicyLocation} options={[{ value: "unselected", label: "Selecciona ubicación", disabled: true }, ...accessibleLocations.map((location) => ({ value: location.id, label: `${location.external_code} · ${location.name}` }))]} disabled={draftLines.length > 0} /></label></div>
      <label ref={productPickerRef} className="inventory-transfer-product-search">Agregar productos<InventoryInput role="combobox" aria-expanded={productPickerOpen} aria-controls="inventory-replenishment-product-options" aria-label="Buscar productos para reabastecimiento" value={productQuery} disabled={!policyLocationId || draftLines.length >= 500} onFocus={() => setProductPickerOpen(true)} onClick={() => setProductPickerOpen(true)} onChange={(event) => { setProductQuery(event.target.value); setProductPickerOpen(true); }} placeholder={policyLocationId ? "Buscar por producto, SKU, código o grupo" : "Selecciona primero la ubicación"} />{policyLocationId && productPickerOpen && <div id="inventory-replenishment-product-options" className="inventory-transfer-product-results inventory-replenishment-product-results" role="listbox">{productSearching ? <p>Buscando productos…</p> : productResults.length ? <><div className="inventory-replenishment-result-actions"><InventoryButton variant="ghost" size="sm" onClick={() => setProductPickerOpen(false)}>Cerrar</InventoryButton><InventoryButton variant="ghost" size="sm" onClick={selectVisibleProducts}>Seleccionar resultados</InventoryButton><InventoryButton variant="primary" size="sm" disabled={!selectedProductIds.size} onClick={addSelectedProducts}>Agregar seleccionados ({selectedProductIds.size})</InventoryButton></div>{productResults.map((product) => { const alreadyAdded = draftLines.some((line) => line.product_id === product.product_id); const selected = selectedProductIds.has(product.product_id); return <label className={alreadyAdded ? "is-disabled" : ""} key={product.product_id}><input type="checkbox" checked={selected || alreadyAdded} disabled={alreadyAdded} onChange={() => toggleProduct(product.product_id)} /><span><strong>{product.product_name}</strong><small>{product.product_code} · {product.unit ?? "Sin unidad"}{product.product_group ? ` · ${product.product_group}` : ""}</small></span><span><b>{numberFormat(Number(product.quantity_on_hand))}</b><small>{product.has_policy ? `Min ${numberFormat(Number(product.minimum_quantity))} · Max ${numberFormat(Number(product.maximum_quantity))}` : "Sin política"}</small></span></label>; })}</> : <p>No hay productos para esta búsqueda.</p>}</div>}</label>
      {draftLines.length ? <><div className="inventory-replenishment-bulk-limits"><label>Mínimo para todas<InventoryInput type="number" min="0.000001" step="0.000001" value={bulkMinimum} onChange={(event) => setBulkMinimum(event.target.value)} /></label><label>Máximo para todas<InventoryInput type="number" min="0.000001" step="0.000001" value={bulkMaximum} onChange={(event) => setBulkMaximum(event.target.value)} /></label><InventoryButton variant="secondary" disabled={!bulkMinimum || !bulkMaximum} onClick={applyBulkLimits}>Aplicar a todas</InventoryButton></div><div className="table-wrap inventory-transfer-draft-lines inventory-replenishment-draft-lines"><table><thead><tr><th>Producto</th><th className="number-cell">Existencia</th><th className="number-cell">Mínimo</th><th className="number-cell">Máximo</th><th aria-label="Acciones" /></tr></thead><tbody>{draftLines.map((line) => { const minimum = Number(line.minimum); const maximum = Number(line.maximum); const invalid = !Number.isFinite(minimum) || minimum <= 0 || !Number.isFinite(maximum) || maximum < minimum; return <tr key={line.product_id}><td><strong>{line.product_name}</strong><small>{line.product_code} · {line.unit ?? "Sin unidad"}</small></td><td className="number-cell">{numberFormat(Number(line.quantity_on_hand))}</td><td className="number-cell"><InventoryInput aria-label={`Mínimo de ${line.product_name}`} aria-invalid={invalid} type="number" min="0.000001" step="0.000001" value={line.minimum} onChange={(event) => updateDraftPolicy(line.product_id, "minimum", event.target.value)} /></td><td className="number-cell"><InventoryInput aria-label={`Máximo de ${line.product_name}`} aria-invalid={invalid} type="number" min="0.000001" step="0.000001" value={line.maximum} onChange={(event) => updateDraftPolicy(line.product_id, "maximum", event.target.value)} />{invalid && <small className="inventory-transfer-line-error">Revisa mínimo y máximo</small>}</td><td><InventoryButton variant="ghost" size="sm" onClick={() => setDraftLines((current) => current.filter((item) => item.product_id !== line.product_id))}>Quitar</InventoryButton></td></tr>; })}</tbody></table></div></> : <div className="inventory-transfer-builder-empty"><strong>Aún no hay productos</strong><span>Busca y selecciona varios productos para configurar sus políticas.</span></div>}
      <footer><span><strong>{draftLines.length}</strong> de 500 políticas{(!policyLocationId || !draftLines.length || invalidDraftLines.length > 0) && <small id="replenishment-policy-requirement" className="inventory-replenishment-requirement">{!policyLocationId ? "Selecciona una ubicación para habilitar el guardado." : !draftLines.length ? "Agrega al menos un producto para habilitar el guardado." : "Corrige los mínimos y máximos marcados antes de guardar."}</small>}</span><div>{draftLines.length > 0 && <InventoryButton variant="secondary" disabled={busy} onClick={() => { setDraftLines([]); setProductQuery(""); setSelectedProductIds(new Set()); }}>Limpiar</InventoryButton>}<InventoryButton variant="primary" loading={busy} aria-describedby={!policyLocationId || !draftLines.length || invalidDraftLines.length > 0 ? "replenishment-policy-requirement" : undefined} disabled={!policyLocationId || !draftLines.length || invalidDraftLines.length > 0} onClick={() => void savePolicyItems()}>Guardar políticas</InventoryButton></div></footer>
    </section></div></InventoryDrawer>}
    {createdRequisition && <section className="inventory-replenishment-created" role="status"><div><strong>Solicitud {createdRequisition.folio} creada</strong><span>{createdRequisition.lineCount} faltantes quedaron listos para cotizar.</span></div><div>{canViewProcurement && <InventoryButton variant="primary" size="sm" onClick={() => openCreatedRequisition(createdRequisition.id)}>Ver solicitud</InventoryButton>}<InventoryButton variant="secondary" size="sm" onClick={() => setCreatedRequisition(null)}>Seguir revisando</InventoryButton></div></section>}
    <section className="inventory-replenishment-work-status" aria-label="Seguimiento de faltantes"><span className="eyebrow">Seguimiento de faltantes</span><div className="bi-segmented inventory-replenishment-status-tabs" role="group" aria-label="Filtrar faltantes por seguimiento">{workStatusOptions.map((option) => <InventoryButton variant="ghost" type="button" key={option.value} className={workStatus === option.value ? "is-active" : ""} aria-pressed={workStatus === option.value} onClick={() => selectWorkStatus(option.value)}><span>{option.label}</span><b>{numberFormat(statusCounts[option.value])}</b></InventoryButton>)}</div></section>
    <InventoryDataToolbar search={search} onSearchChange={setSearch} placeholder="Buscar producto o SKU" filters={<><InventorySelect value={locationFilter} onValueChange={(value) => { setLocationFilter(value); setPage(1); }} ariaLabel="Filtrar reabastecimiento por ubicación" options={locationOptions} /><InventorySelect value={belowMinimumOnly ? "below" : "all"} onValueChange={(value) => { setBelowMinimumOnly(value === "below"); setPage(1); }} ariaLabel="Mostrar políticas" options={[{ value: "below", label: "Solo bajo mínimo" }, { value: "all", label: "Todas las políticas" }]} /></>} activeFilters={(search.trim() ? 1 : 0) + (locationFilter !== "all" ? 1 : 0) + (belowMinimumOnly ? 0 : 1) + (workStatus !== "unattended" ? 1 : 0)} onClear={clearFilters} results={total} />
    <DataState preserveData loading={loading} error={error} errorAction={<InventoryButton size="sm" onClick={() => void load()}>Reintentar</InventoryButton>} hasData={rows.length} emptyTitle={workStatus === "unattended" ? "No hay faltantes sin atender." : "No hay resultados en este seguimiento."} empty={empty}><div className="table-wrap surface-table"><table><thead><tr><th>Producto</th><th>Ubicación</th><th className="number-cell">Existencia</th><th className="number-cell">Mínimo</th><th className="number-cell">Máximo</th><th className="number-cell">Sugerido</th><th>Seguimiento</th><th aria-label="Acciones" /></tr></thead><tbody>{rows.map((row) => <tr key={row.policy_id}><td><strong>{row.product_name}</strong><small>{row.product_code} · {row.unit ?? "Sin unidad"}</small></td><td><span className="location-chip">{row.location_code}</span> {row.location_name}</td><td className="number-cell">{numberFormat(Number(row.quantity_on_hand))}</td><td className="number-cell">{numberFormat(Number(row.minimum_quantity))}</td><td className="number-cell">{numberFormat(Number(row.maximum_quantity))}</td><td className="number-cell"><strong>{row.is_below_minimum ? numberFormat(Number(row.suggested_quantity)) : "—"}</strong>{row.is_below_minimum && <small>Faltan {numberFormat(Number(row.shortage_quantity))} al mínimo</small>}</td><td><Badge tone={row.work_status === "unattended" ? "warning" : row.work_status === "in_transit" ? "success" : "neutral"}>{row.work_status_label}</Badge>{row.requisition_folio && <small className="inventory-replenishment-reference">{row.requisition_folio}</small>}</td><td>{row.work_status === "unattended" && canPrepareRequisition ? <InventoryButton variant="secondary" size="sm" onClick={() => createForLocation(row.location_id)}>Crear solicitud</InventoryButton> : row.requisition_id && canViewProcurement ? <InventoryButton variant="secondary" size="sm" onClick={() => openCreatedRequisition(row.requisition_id)}>Ver solicitud</InventoryButton> : <span className="table-muted">—</span>}</td></tr>)}</tbody></table></div></DataState><DataPagination page={page} total={total} pageSize={DATA_PAGE_SIZE} onChange={setPage} />
    <InventoryModal open={bulkImportOpen} onOpenChange={(open) => !busy && setBulkImportOpen(open)} eyebrow="Carga secundaria" title="Importar políticas por SKU" description="Úsalo para pegar datos preparados en Excel. Para operación diaria utiliza la selección visual." footer={<><InventoryButton variant="secondary" disabled={busy} onClick={() => setBulkImportOpen(false)}>Cancelar</InventoryButton><InventoryButton variant="primary" loading={busy} disabled={!policyLocationId || !batch.trim()} onClick={() => void importPolicies()}>Importar y guardar</InventoryButton></>}><label className="operation-reason">Una política por renglón: SKU,mínimo,máximo<textarea value={batch} onChange={(event) => setBatch(event.target.value)} rows={7} placeholder={"FERT-001,10,30\nRIEGO-020,5,15"} /></label><small>Se validarán entre 1 y 500 SKU distintos antes de guardar.</small></InventoryModal>
    <InventoryModal open={requisitionConfirmationOpen} onOpenChange={(open) => !busy && setRequisitionConfirmationOpen(open)} eyebrow="Solicitud de compra" title="Crear solicitud de compra" description={`Se creará una solicitud para todos los faltantes elegibles de ${selectedReplenishmentLocation ? `${selectedReplenishmentLocation.external_code} · ${selectedReplenishmentLocation.name}` : "la ubicación seleccionada"}. La búsqueda y los filtros de productos no limitan esta solicitud.`} footer={<><InventoryButton variant="secondary" disabled={busy} onClick={() => setRequisitionConfirmationOpen(false)}>Volver</InventoryButton><InventoryButton variant="primary" loading={busy} disabled={locationFilter === "all"} onClick={() => void prepareRequisition()}>Crear solicitud</InventoryButton></>}><p className="settings-note">Podrás abrir la solicitud en Compras para cotizar y decidir la compra después; no se creará una orden de compra.</p></InventoryModal>
  </div>;
}
...
function InventoryTransfersView({ companyId, permissions }: { companyId: string; permissions: string[] }) {
  ...
  const loadTransfersRequest = useRef(0);
  const loadTransfers = useCallback(async () => {
    const request = ++loadTransfersRequest.current;
    setLoading(true); setError(null);
    ...
    if (request !== loadTransfersRequest.current) return;
  ...
  const loadLinesRequest = useRef(0);
  const loadLines = useCallback(async () => {
    const request = ++loadLinesRequest.current;
    ...
    if (request !== loadLinesRequest.current) return;
  ...
  async function submitTransferItems(items: Array<{ product_id: string; quantity: number }>) {
    ...
    if (rpcError) toast({ title: "No se preparó la transferencia", description: inventoryRpcMessage(rpcError, "Revisa existencias y cantidades."), tone: "error" });
      ...
      setBuilderOpen(false); setSelectedTransferId(result.inventory_transfer_id); setLinePage(1); setDraftLines([]); setProductQuery(""); setStatusFilter("all");
      await loadTransfers();
...
function InventoryCountsView({ companyId, permissions }: { companyId: string; permissions: string[] }) {
  ...
  const loadCountsRequest = useRef(0);
  const loadCounts = useCallback(async () => {
    const request = ++loadCountsRequest.current;
    setLoading(true); setError(null);
    ...
    if (request !== loadCountsRequest.current) return;
  ...
  const countAction = selectedCount
    ? <InventoryButton variant="secondary" disabled={busy} onClick={() => void closeWorkspace()}>Volver a conteos</InventoryButton>
    : canOperate ? <div className="inventory-count-open"><InventorySelect ariaLabel="Ubicación para nuevo conteo" value={newLocationId || "unselected"} onValueChange={(value) => setNewLocationId(value === "unselected" ? "" : value)} options={[{ value: "unselected", label: "Selecciona ubicación", disabled: true }, ...accessibleLocations.map((location) => ({ value: location.id, label: `${location.external_code} · ${location.name}` }))]} /><InventoryButton variant="primary" loading={busy} disabled={!newLocationId} onClick={() => void openCount()}>Abrir conteo</InventoryButton></div> : undefined;
  ...
  return <div className="content-frame inventory-counts"><PageHeading eyebrow="Control físico" title={selectedCount ? selectedCount.location_name : "Conteos físicos"} description={selectedCount ? `Conteo ${selectedCount.location_code} · captura ciega, guardada por lotes y auditable.` : "Captura un conteo completo por ubicación. Las diferencias solo afectan inventario después de una aprobación independiente."} action={countAction} />
    {!selectedCount ? <>
      ...
      {(selectedCount.status === "open" || selectedCount.status === "review" || selectedCount.status === "pending_approval") && <div className="inventory-count-actions">
        {selectedCount.status === "review" && <label>Explicación de las diferencias<InventoryInput value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Describe la causa o el resultado de la verificación" /></label>}
        {selectedCount.status === "pending_approval" && <label>Motivo de la decisión<InventoryInput value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Obligatorio para rechazar" /></label>}
        <div>{selectedCount.status === "open" && canOperate && <><InventoryButton variant="danger" disabled={busy} onClick={() => setCancelReason("")}>Cancelar conteo</InventoryButton><InventoryButton loading={busy} onClick={() => void savePage()}>Guardar avance</InventoryButton><InventoryButton variant="primary" loading={busy} onClick={() => void reviewCount()}>Finalizar captura</InventoryButton></>}{selectedCount.status === "review" && canOperate && <><InventoryButton variant="danger" disabled={busy} onClick={() => setCancelReason("")}>Cancelar conteo</InventoryButton><InventoryButton variant="primary" loading={busy} disabled={!reason.trim()} onClick={() => void submitCount()}>Enviar diferencias a aprobación</InventoryButton></>}{selectedCount.status === "pending_approval" && canApprove && <><InventoryButton variant="danger" loading={busy} onClick={() => void decideCount(false)}>Rechazar</InventoryButton><InventoryButton variant="primary" loading={busy} onClick={() => void decideCount(true)}>Aprobar y ajustar</InventoryButton></>}</div>
      </div>}
    ...
    <InventoryModal open={cancelReason !== null} onOpenChange={(open) => { if (!open && !busy) setCancelReason(null); }} eyebrow="Sin afectar inventario" title="Cancelar conteo" description="La captura quedará auditada, la ubicación volverá a estar disponible y no se generará ningún movimiento." footer={<><InventoryButton variant="secondary" disabled={busy} onClick={() => setCancelReason(null)}>Volver</InventoryButton><InventoryButton variant="danger" loading={busy} disabled={!cancelReason?.trim()} onClick={() => void cancelCount()}>Confirmar cancelación</InventoryButton></>}><label className="operation-reason">Motivo obligatorio<textarea rows={4} value={cancelReason ?? ""} onChange={(event) => setCancelReason(event.target.value)} placeholder="Ej. Conteo abierto para una prueba" /></label></InventoryModal>
    <InventoryModal open={confirmVisibleZero} onOpenChange={(open) => !busy && setConfirmVisibleZero(open)} eyebrow="Confirmación física" title="Confirmar partidas visibles en cero" description={`Se guardarán ${pendingVisible} partidas pendientes de esta página en un solo lote. Úsalo únicamente después de comprobar físicamente que no existe producto.`} footer={<><InventoryButton variant="secondary" disabled={busy} onClick={() => setConfirmVisibleZero(false)}>Volver</InventoryButton><InventoryButton variant="primary" loading={busy} onClick={() => void markVisiblePendingAsZero()}>Confirmar en cero</InventoryButton></>} />
  </div>;
}
