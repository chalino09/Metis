...
import { DataPagination, DataRefreshStatus, DataState, InteractiveTableRow, PagedCollection, Table } from "@/app/components/ui/data";
...
import { PurchasingDrawer as Drawer, PurchasingModal as Modal, PurchasingTabs as Tabs } from "@/app/components/reui/purchasing-panels";
...
type ReceiptCandidate = { id: string; receipt_folio: string; receipt_date: string; purchase_order_id: string; purchase_order_folio: string; currency_code: string; supplier_id: string; supplier_name: string; payable_term_days: number | null };
type SupplierOption = { id: string; code: string; display_name: string; tax_id: string | null; payable_term_days: number | null };
...
type Detail = InvoiceRow & {
  ...
  supplier_payable_term_days_snapshot: number | null; due_date_source: "supplier_terms" | "manual_override" | "legacy";
  due_date_override_reason: string | null;
  on_time_total_snapshot: number | null; late_payment_total: number | null; payment_terms_evidence: string | null;
...
type PromptPaymentOption = { tier_number:number;term_days:number;deadline:string;discount_expression:string;effective_discount_percent:number;estimated_total:number;estimated_savings?:number };
type DuePayableRow = {
  ...
  prompt_payment_terms: PromptPaymentOption[]; eligible_prompt_payment: PromptPaymentOption | null;
};
...
type PaymentCalendarRow = {
  ...
  eligible_prompt_payment: PromptPaymentOption | null;
};
...
type Action = { kind: "authorize" | "approve_expense" | "reverse" | "credit" | "sat"; reason: string; evidence: string; amount: string; series: string; folio: string; satStatus: "valid" | "cancelled" | "not_found" };
...
const suggestedProposalAmount = (row: DuePayableRow) => row.eligible_prompt_payment?.estimated_total ?? row.outstanding_amount;
const promptPaymentBreakdown = (option: PromptPaymentOption, amount: number, currency: string) => `${option.discount_expression}: ahorro ${money(amount - option.estimated_total, currency)}`;
...
export function SupplierInvoicesView({ companyId, permissions }: { companyId: string; permissions: string[] }) {
  ...
  const load = useCallback(async () => {
    ...
    if (tab === "invoices") {
      ...
      const [agingResult, dueResult] = await Promise.all([
        ...
        getSupabaseClient().rpc("search_supplier_payable_due_inbox_v2", {
  ...
  function openNew() {
    ...
    setDraft({ sourceKind: "receipt", receiptId: "", supplierId: "", series: "", folio: "", fiscalUuid: "", issuedDate: today(), dueDate: today(), supplierTermDays: null, dueDateOverrideReason: "", currencyCode: "MXN", exchangeRate: "1", reference: "", paymentMethodCode: "PPD", paymentFormCode: "99", quantities: {}, prices: {}, discounts: {}, taxes: {}, expenseLines: [blankExpenseLine()] });
  }
  ...
  async function chooseReceipt(receiptId: string) {
    ...
    setDraft(current => current ? {
      ...
      supplierTermDays: receipt.payable_term_days ?? null,
      dueDate: receipt.payable_term_days == null ? current.issuedDate : addDays(current.issuedDate, receipt.payable_term_days),
      dueDateOverrideReason: "",
      currencyCode: next.currency_code,
      exchangeRate: "1",
      prices: Object.fromEntries(next.lines.map(line => [line.purchase_receipt_line_id, String(line.received_unit_cost)])),
    } : current);
  }
  ...
  async function readDraftXml(file: File) {
    ...
    try {
      ...
      setDraft(current => current ? {
        ...
        supplierTermDays: supplier ? supplier.payable_term_days ?? null : current.supplierTermDays,
        series: extracted.series ?? "",
        folio: extracted.folio ?? "",
        fiscalUuid: extracted.uuid ?? "",
        issuedDate,
        dueDate: supplier?.payable_term_days == null
          ? issuedDate
          : addDays(issuedDate, supplier?.payable_term_days ?? current.supplierTermDays ?? 0),
        dueDateOverrideReason: "",
        currencyCode: extracted.currency ?? current.currencyCode,
        paymentMethodCode: extracted.payment_method_code ?? current.paymentMethodCode,
        paymentFormCode: extracted.payment_form_code ?? current.paymentFormCode,
        expenseLines: expenseLines.length ? expenseLines : current.expenseLines,
      } : current);
  ...
  async function saveDraft() {
    ...
    const calculatedDueDate = draft.supplierTermDays === null ? null : addDays(draft.issuedDate, draft.supplierTermDays);
    const needsDueDateOverride = calculatedDueDate === null || draft.dueDate !== calculatedDueDate;
    if (needsDueDateOverride && !draft.dueDateOverrideReason.trim()) {
      toast({ title: "Documenta el vencimiento", description: calculatedDueDate === null ? "El proveedor aún no tiene plazo configurado; explica por qué se usará esta fecha." : "La fecha no coincide con el plazo del proveedor; explica la excepción.", tone: "info" });
      return;
    }
    ...
    if (draft.sourceKind === "receipt") {
      ...
      ({ data, error: rpcError } = await getSupabaseClient().rpc("save_supplier_invoice_v3", { p_company_id: companyId, p_invoice_id: null, p_supplier_id: candidate.supplier_id, p_purchase_order_id: candidate.purchase_order_id, p_series: draft.series || null, p_folio: draft.folio, p_fiscal_uuid: draft.fiscalUuid || null, p_issued_date: draft.issuedDate, p_requested_due_date: draft.dueDate, p_currency_code: draft.currencyCode.toUpperCase(), p_exchange_rate: Number(draft.exchangeRate), p_supplier_reference: draft.reference || null, p_payment_method_code: draft.paymentMethodCode || null, p_payment_form_code: draft.paymentFormCode || null, p_lines: lines, p_late_payment_total: null, p_payment_terms_evidence: null, p_due_date_override_reason: draft.dueDateOverrideReason.trim() || null, p_client_request_id: keys.current.get("supplier-invoice-save", fingerprint), p_expected_updated_at: null }));
    } else {
      ...
      ({ data, error: rpcError } = await getSupabaseClient().rpc("save_supplier_expense_invoice_v2", { p_company_id: companyId, p_invoice_id: null, p_supplier_id: draft.supplierId, p_series: draft.series || null, p_folio: draft.folio, p_fiscal_uuid: draft.fiscalUuid || null, p_issued_date: draft.issuedDate, p_requested_due_date: draft.dueDate, p_currency_code: draft.currencyCode.toUpperCase(), p_exchange_rate: Number(draft.exchangeRate), p_supplier_reference: draft.reference || null, p_payment_method_code: draft.paymentMethodCode || null, p_payment_form_code: draft.paymentFormCode || null, p_lines: lines, p_late_payment_total: null, p_payment_terms_evidence: null, p_due_date_override_reason: draft.dueDateOverrideReason.trim() || null, p_expected_updated_at: null }));
    }
    setBusy(false);
  ...
  function togglePayable(row: DuePayableRow) {
    ...
    setProposedAmounts({ ...proposedAmounts, [row.id]: String(suggestedProposalAmount(row)) });
  }
  ...
  async function saveProposal() {
    ...
    if (lines.some((line, index) => !Number.isFinite(line.proposed_amount) || line.proposed_amount <= 0 || line.proposed_amount > selected[index].outstanding_amount || Math.abs(line.proposed_amount * 100 - Math.round(line.proposed_amount * 100)) > 1e-8)) {
      toast({ title: "Revisa los importes", description: "Cada importe debe ser positivo, no superar el saldo y usar dos decimales.", tone: "info" }); return;
    }
  ...
  async function openCalendarItem(item: PaymentCalendarRow) {
    ...
    const payable: DuePayableRow = {
      ...
      prompt_payment_terms: [], eligible_prompt_payment: item.eligible_prompt_payment,
    };
    setSelectedPayables({ [item.id]: payable }); setProposedAmounts({ [item.id]: String(suggestedProposalAmount(payable)) });
    setProposalDraftId(null); setProposalExpectedUpdatedAt(null); setProposalBuilderOpen(true);
  }
  ...
  function editProposal(detail: ProposalDetail) {
    const rows = Object.fromEntries(detail.lines.map(line => [line.accounts_payable_id, {
      ...
      prompt_payment_terms: [], eligible_prompt_payment: null,
  ...
  const supplierFilterPicker = <div className="purchase-order-supplier-picker">
    ...
    {filterSupplierPickerOpen && <div className="purchase-order-supplier-options" role="listbox">
      <Button variant="ghost" type="button" role="option" aria-selected={!dueSupplier} onMouseDown={event => event.preventDefault()} onClick={() => { setDueSupplier(""); setFilterSupplierQuery(""); setFilterSupplierPickerOpen(false); setPage(1); }}><strong>Todos los proveedores</strong><small>Sin filtro de proveedor</small></Button>
      {filterSuppliers.map(item => <Button variant="ghost" type="button" role="option" aria-selected={dueSupplier === item.id} key={item.id} onMouseDown={event => event.preventDefault()} onClick={() => { setDueSupplier(item.id); setFilterSupplierQuery(`${item.code} · ${item.display_name}`); setFilterSupplierPickerOpen(false); setPage(1); }}><strong>{item.display_name}</strong><small>{item.code}{item.tax_id ? ` · ${item.tax_id}` : ""}</small></Button>)}
  ...
  return <div className="content-frame supplier-invoice-module" data-active-tab={tab}>
    <div className="page-heading"><div><span className="eyebrow">Compras</span><h1>Facturas y CxP</h1><p>Registra factura o gasto, confirma la cuenta por pagar y administra pronto pago, abonos, pagos y reportes desde el mismo espacio.</p></div><div className="page-heading-actions"><Button loading={loading} onClick={() => void load()}><RefreshCw size={16} /> Actualizar</Button>{tab === "payables" && payablesSection === "current" && canPrepareProposals && Object.keys(selectedPayables).length > 0 && <Button variant="primary" onClick={openProposalBuilder}>Crear propuesta ({Object.keys(selectedPayables).length})</Button>}{(canDraft || canExpense) && tab === "invoices" && <Button variant="primary" onClick={() => void openNew()}><FilePlus2 size={16} /> Nueva factura</Button>}</div></div>
    <Tabs className="supplier-invoice-primary-tabs" ariaLabel="Secciones de Facturas y CxP" value={tab} onValueChange={changeTab} items={[{ value: "invoices", label: "Facturas" }, { value: "payables", label: "Cuentas por pagar" }, ...((canPrepareProposals || canApproveProposals || canViewPayments) ? [{ value: "payments", label: "Pagos" }] : [])]} />
    {tab === "payables" && <Tabs className="supplier-invoice-secondary-tabs" ariaLabel="Secciones de cuentas por pagar" value={payablesSection} onValueChange={value => { setPayablesSection(value as "current" | "history"); setQuery(""); setPage(1); }} items={[{ value: "current", label: "Por pagar" }, { value: "history", label: "Historial" }]} />}
    {tab === "payments" && <Tabs className="supplier-invoice-secondary-tabs" ariaLabel="Secciones de pagos" value={paymentsSection} onValueChange={value => { setPaymentsSection(value as "proposals" | "agenda" | "confirmed"); setStatus("all"); setPage(1); }} items={[...((canPrepareProposals || canApproveProposals) ? [{ value: "proposals", label: "Propuestas" }] : []), { value: "agenda", label: "Agenda" }, ...(canViewPayments ? [{ value: "confirmed", label: "Pagos realizados" }] : [])]} />}
    ...
    {tab === "invoices" && <DataToolbar search={query} onSearchChange={value => { setQuery(value); setPage(1); }} placeholder="Proveedor, folio, UUID u orden de compra" results={total} activeFilters={status === "all" ? 0 : 1} onClear={() => setStatus("all")} filters={<Select ariaLabel="Estado" value={status} onValueChange={value => { setStatus(value); setPage(1); }} options={statusOptions} />} />}
    <DataRefreshStatus loading={loading} hasData={visibleDataCount} />
    {tab === "invoices" && <span className="supplier-invoice-exception-state" data-count={exceptionTotal} aria-hidden="true" />}

    {tab === "invoices" && <><div className="supplier-invoice-exception-summary"><Button size="sm" aria-expanded={showExceptions} onClick={() => setShowExceptions(value => !value)}><AlertTriangle size={15} /> Excepciones ({exceptionTotal})</Button><span>Alertas documentales integradas; no alteran el estado de la factura.</span></div>{showExceptions && <DataState loading={loading} error={error} hasData={exceptions.length} empty="No hay excepciones registradas."><div className="supplier-invoice-exceptions">{exceptions.map(item => <article key={item.id}><AlertTriangle size={18} /><span><strong>{exceptionLabel(item.kind)} · {item.supplier_name}</strong><small>{item.invoice_folio ? `Factura ${item.invoice_folio}` : "Documento bloqueado antes de crearse"} · {new Date(item.detected_at).toLocaleString("es-MX")}</small></span><Badge tone={item.status === "pending" ? "warning" : "success"}>{item.status === "pending" ? "Pendiente" : "Resuelta"}</Badge></article>)}</div><Pagination page={exceptionPage} total={exceptionTotal} setPage={setExceptionPage} /></DataState>}<DataState loading={loading} error={error} hasData={rows.length} emptyTitle="No hay facturas." empty="No hay facturas con estos filtros."><Table><thead><tr><th>Factura</th><th>Proveedor / origen</th><th>Emisión / vencimiento</th><th>Diferencias</th><th>Estado</th><th className="number-cell">Total / saldo</th><th aria-label="Abrir detalle" style={{ width: 40 }}></th></tr></thead><tbody>{rows.map(row => <InteractiveTableRow key={row.id} className="purchase-order-row" label={`Abrir factura ${[row.series, row.folio].filter(Boolean).join("-")}`} onActivate={() => void openDetail(row.id)}><td><strong>{[row.series, row.folio].filter(Boolean).join("-")}</strong><small>{row.fiscal_uuid ?? "Sin UUID fiscal"}</small></td><td><strong>{row.supplier_name}</strong><small>{row.purchase_order_folio ?? "Gasto o servicio sin recepción"}</small></td><td>{date(row.issued_date)}<small>{date(row.due_date)}</small></td><td>{row.differences.length ? <Badge tone="warning">{row.differences.length} por revisar</Badge> : <Badge tone="success">Sin diferencias</Badge>}</td><td><InvoiceBadge status={row.status} /></td><td className="number-cell"><strong>{money(row.total, row.currency_code)}</strong><small>{row.outstanding_amount === null ? "Sin CxP" : `Saldo ${money(row.outstanding_amount, row.currency_code)}`}</small></td><td aria-hidden="true" style={{ width: 40, color: "var(--muted)", textAlign: "right" }}><ChevronRight size={18} strokeWidth={2} /></td></InteractiveTableRow>)}</tbody></Table><Pagination page={page} total={total} setPage={setPage} /></DataState></>}
    {tab === "payables" && payablesSection === "current" && <><section className="supplier-payables-section"><section className="supplier-payments-list-heading"><h2>Resumen de antigüedad</h2><p>Saldos abiertos agrupados por moneda y días de vencimiento.</p></section><DataState loading={loading} error={error} hasData={aging.length} empty="No hay saldos abiertos para calcular antigüedad."><div className="payables-aging-grid" aria-label="Resumen de antigüedad">{aging.map(item => <article key={item.currency_code}><header><span>{item.document_count} documentos</span><strong>{money(item.total, item.currency_code)}</strong><Badge tone="info">{item.currency_code}</Badge></header><dl><div><dt>Por vencer</dt><dd>{money(item.not_due, item.currency_code)}</dd></div><div><dt>1–30 días</dt><dd>{money(item.days_1_30, item.currency_code)}</dd></div><div><dt>31–60 días</dt><dd>{money(item.days_31_60, item.currency_code)}</dd></div><div><dt>61–90 días</dt><dd>{money(item.days_61_90, item.currency_code)}</dd></div><div><dt>Más de 90</dt><dd>{money(item.days_over_90, item.currency_code)}</dd></div></dl></article>)}</div></DataState></section><section className="supplier-payments-list-heading"><h2>Vencimientos y saldos</h2><p>Filtra y selecciona CxP abiertas para preparar una propuesta sin modificar sus saldos.</p></section><DataState loading={loading} error={error} hasData={duePayables.length} empty="No hay CxP abiertas con estos filtros."><Table><thead><tr>{canPrepareProposals && <th>Elegir</th>}<th>Factura</th><th>Proveedor</th><th>Vencimiento</th><th>Pronto pago</th><th>Clasificación</th><th className="number-cell">Saldo actual</th></tr></thead><tbody>{duePayables.map(row => <tr key={row.id}>{canPrepareProposals && <td><input aria-label={`Seleccionar ${row.invoice_number}`} type="checkbox" checked={Boolean(selectedPayables[row.id])} disabled={Boolean(Object.values(selectedPayables)[0] && (Object.values(selectedPayables)[0].supplier_id !== row.supplier_id || Object.values(selectedPayables)[0].currency_code !== row.currency_code))} onChange={() => togglePayable(row)} /></td>}<td><strong>{row.invoice_number}</strong><small>{date(row.issued_date)}</small></td><td><strong>{row.supplier_name}</strong><small>{row.supplier_code}</small></td><td>{date(row.due_date)}</td><td>{row.eligible_prompt_payment?<><Badge tone="success">{row.eligible_prompt_payment.discount_expression} · {row.eligible_prompt_payment.effective_discount_percent}%</Badge><small>Hasta {date(row.eligible_prompt_payment.deadline)} · pagar {money(row.eligible_prompt_payment.estimated_total,row.currency_code)}</small></>:<span>Sin descuento vigente</span>}</td><td><DueBucketBadge bucket={row.due_bucket} /></td><td className="number-cell"><strong>{money(row.outstanding_amount, row.currency_code)}</strong></td></tr>)}</tbody></Table><Pagination page={page} total={total} setPage={setPage} /></DataState></>}
    ...
    <Drawer open={Boolean(draft)} onOpenChange={open => !open && !busy && setDraft(null)} title="Nueva factura de proveedor" className="purchase-order-drawer supplier-invoice-drawer">{draft && <form className="purchase-order-form" onSubmit={event => { event.preventDefault(); void saveDraft(); }}>
      ...
      <div className="purchasing-form-sections"><Card className="purchasing-form-section"><h3>Origen de la factura</h3><div className="purchasing-form-grid">{draft.sourceKind === "receipt" ? <Field label="Recepción confirmada" hint={receiptSearching ? "Buscando…" : `${receiptTotal} recepciones disponibles; busca por recepción, orden de compra o proveedor.`}><div className="purchase-order-supplier-picker"><Input role="combobox" aria-label="Recepción confirmada" aria-expanded={receiptPickerOpen} autoComplete="off" value={receiptQuery} placeholder="Buscar recepción confirmada" onFocus={() => setReceiptPickerOpen(true)} onBlur={() => window.setTimeout(() => setReceiptPickerOpen(false), 120)} onChange={event => { setReceiptQuery(event.target.value); setReceiptPickerOpen(true); if (draft.receiptId) { setDraft({ ...draft, receiptId: "", supplierId: "", supplierTermDays: null, dueDateOverrideReason: "" }); setCandidate(null); } }} />{receiptPickerOpen && <div className="purchase-order-supplier-options" role="listbox">{receipts.map(item => <Button variant="ghost" type="button" role="option" aria-selected={draft.receiptId === item.id} key={item.id} onMouseDown={event => event.preventDefault()} onClick={() => { setReceiptQuery(`${item.receipt_folio} · ${item.purchase_order_folio} · ${item.supplier_name}`); setReceiptPickerOpen(false); void chooseReceipt(item.id); }}><strong>{item.receipt_folio} · {item.purchase_order_folio}</strong><small>{item.supplier_name} · {date(item.receipt_date)} · {item.payable_term_days == null ? "Sin plazo" : `${item.payable_term_days} días`}</small></Button>)}{!receiptSearching && receipts.length === 0 && <p>No se encontraron recepciones facturables.</p>}</div>}</div></Field> : <Field label="Proveedor" hint={draftSupplierSearching ? "Buscando…" : `${draftSupplierTotal} proveedores activos; busca por nombre, clave o RFC.`}><div className="purchase-order-supplier-picker"><Input role="combobox" aria-label="Proveedor" aria-expanded={draftSupplierPickerOpen} autoComplete="off" value={draftSupplierQuery} placeholder="Buscar proveedor" onFocus={() => setDraftSupplierPickerOpen(true)} onBlur={() => window.setTimeout(() => setDraftSupplierPickerOpen(false), 120)} onChange={event => { setDraftSupplierQuery(event.target.value); setDraftSupplierPickerOpen(true); if (draft.supplierId) setDraft({ ...draft, supplierId: "", supplierTermDays: null, dueDateOverrideReason: "" }); }} />{draftSupplierPickerOpen && <div className="purchase-order-supplier-options" role="listbox">{suppliers.map(item => <Button variant="ghost" type="button" role="option" aria-selected={draft.supplierId === item.id} key={item.id} onMouseDown={event => event.preventDefault()} onClick={() => { setDraftSupplierQuery(`${item.code} · ${item.display_name}`); setDraftSupplierPickerOpen(false); setDraft({ ...draft, supplierId: item.id, supplierTermDays: item.payable_term_days ?? null, dueDate: item.payable_term_days == null ? draft.issuedDate : addDays(draft.issuedDate, item.payable_term_days), dueDateOverrideReason: "" }); }}><strong>{item.display_name}</strong><small>{item.code}{item.tax_id ? ` · ${item.tax_id}` : ""} · {item.payable_term_days == null ? "Sin plazo" : `${item.payable_term_days} días`}</small></Button>)}{!draftSupplierSearching && suppliers.length === 0 && <p>No se encontraron proveedores activos.</p>}</div>}</div></Field>}</div></Card><Card className="purchasing-form-section"><h3>Identificación del documento</h3><div className="purchasing-form-grid"><Field label="Serie"><Input value={draft.series} onChange={event => setDraft({ ...draft, series: event.target.value })} /></Field><Field label="Folio"><Input required value={draft.folio} onChange={event => setDraft({ ...draft, folio: event.target.value })} /></Field><Field label="UUID fiscal" hint="Se valida nuevamente al adjuntar XML."><Input value={draft.fiscalUuid} onChange={event => setDraft({ ...draft, fiscalUuid: event.target.value })} /></Field><Field label="Referencia"><Input value={draft.reference} onChange={event => setDraft({ ...draft, reference: event.target.value })} /></Field></div></Card><Card className="purchasing-form-section"><h3>Fechas y condiciones fiscales</h3><div className="purchasing-form-grid"><Field label="Emisión"><Input type="date" value={draft.issuedDate} onChange={event => setDraft({ ...draft, issuedDate: event.target.value, dueDate: draft.supplierTermDays === null ? event.target.value : addDays(event.target.value, draft.supplierTermDays), dueDateOverrideReason: "" })} /></Field><Field label="Vencimiento" hint={draft.supplierTermDays === null ? "Falta configurar el plazo del proveedor; esta fecha será una excepción auditada." : `Calculado automáticamente: ${draft.supplierTermDays} días naturales desde la emisión.`}><Input type="date" min={draft.issuedDate} value={draft.dueDate} onChange={event => setDraft({ ...draft, dueDate: event.target.value, dueDateOverrideReason: event.target.value === (draft.supplierTermDays === null ? "" : addDays(draft.issuedDate, draft.supplierTermDays)) ? "" : draft.dueDateOverrideReason })} /></Field><Field label="Moneda"><div className="invoice-currency-field"><Select ariaLabel="Moneda" value={COMMON_CURRENCIES.some(option => option.value === draft.currencyCode) ? draft.currencyCode : OTHER_CURRENCY} onValueChange={value => setDraft({ ...draft, currencyCode: value === OTHER_CURRENCY ? "" : value, exchangeRate: value === "MXN" ? "1" : draft.exchangeRate })} options={[...COMMON_CURRENCIES, { value: OTHER_CURRENCY, label: "Otra moneda…" }]} />{!COMMON_CURRENCIES.some(option => option.value === draft.currencyCode) && <Input aria-label="Código de otra moneda" placeholder="Ej. CAD" maxLength={3} value={draft.currencyCode} onChange={event => { const currencyCode = event.target.value.replace(/[^a-zA-Z]/g, "").toUpperCase(); setDraft({ ...draft, currencyCode, exchangeRate: currencyCode === "MXN" ? "1" : draft.exchangeRate }); }} />}</div></Field><Field label="Tipo de cambio" hint="Moneda de factura → moneda base."><Input type="number" min="0.000001" step="0.000001" disabled={draft.currencyCode === "MXN"} value={draft.exchangeRate} onChange={event => setDraft({ ...draft, exchangeRate: event.target.value })} /></Field><Field label="Método CFDI"><Select ariaLabel="Método CFDI" value={draft.paymentMethodCode} onValueChange={value => setDraft({ ...draft, paymentMethodCode: value })} options={PAYMENT_METHODS} /></Field><Field label="Forma CFDI"><Select ariaLabel="Forma CFDI" value={draft.paymentFormCode} onValueChange={value => setDraft({ ...draft, paymentFormCode: value })} options={PAYMENT_FORMS} /></Field></div></Card></div>
      {(draft.supplierTermDays === null || draft.dueDate !== addDays(draft.issuedDate, draft.supplierTermDays)) && <label className="operation-reason">Motivo del vencimiento manual<textarea rows={3} placeholder={draft.supplierTermDays === null ? "Ej. Condición pendiente de homologar con el proveedor" : "Explica por qué esta factura no sigue el plazo configurado"} value={draft.dueDateOverrideReason} onChange={event => setDraft({ ...draft, dueDateOverrideReason: event.target.value })} /></label>}
    ...
    <Drawer open={Boolean(detail)} onOpenChange={open => !open && !busy && setDetail(null)} title={detail ? `Factura ${[detail.series, detail.folio].filter(Boolean).join("-")}` : "Factura"} className="purchase-order-detail-drawer">{detail && <div className="purchase-order-detail supplier-invoice-detail"><Card className="purchasing-document-summary"><header><div><InvoiceBadge status={detail.status} /> <Badge tone={detail.source_kind === "receipt" ? "info" : "neutral"}>{detail.source_kind === "receipt" ? "Contra recepción" : "Gasto/servicio"}</Badge>{detail.differences.length > 0 && <Badge tone={detail.differences_authorized_at ? "success" : "warning"}>{detail.differences_authorized_at ? "Diferencias autorizadas" : "Diferencias pendientes"}</Badge>}</div><strong>{money(detail.total, detail.currency_code)}</strong></header><dl><div><dt>Proveedor</dt><dd>{detail.supplier.display_name} · {detail.supplier.code}</dd></div><div><dt>Origen</dt><dd>{detail.purchase_order ? `Orden de compra ${detail.purchase_order.folio} · ${detail.receipts.map(item => item.folio).join(", ")}` : "Gasto o servicio aprobado sin recepción"}</dd></div><div><dt>Emisión / vencimiento</dt><dd>{date(detail.issued_date)} / {date(detail.due_date)}<small>{detail.due_date_source === "supplier_terms" ? `Calculado con ${detail.supplier_payable_term_days_snapshot ?? 0} días desde emisión` : detail.due_date_source === "manual_override" ? `Excepción auditada: ${detail.due_date_override_reason}` : "Documento anterior a la configuración de plazos"}</small></dd></div><div><dt>Pronto pago</dt><dd>Consulta el beneficio vigente en Pagos → Agenda.<small>La factura conserva las condiciones existentes al crear el borrador.</small></dd></div><div><dt>Moneda / tipo de cambio</dt><dd>{detail.currency_code} · {detail.exchange_rate} → {detail.base_currency_code} ({money(detail.base_total, detail.base_currency_code)})</dd></div><div><dt>CxP</dt><dd>{detail.payable ? `${money(detail.payable.outstanding_amount, detail.currency_code)} · ${conditionLabel(detail.payable.condition)}` : "No generada mientras sea borrador"}</dd></div></dl></Card><PurchasingDocumentTabs key={detail.id} sections={[{ id: "lines", label: "Partidas", content: <>{detail.source_kind === "receipt" ? <PagedCollection items={detail.lines} resetKey={detail.id} label="partidas">{visibleLines=><Table><thead><tr><th>Recepción / partida</th><th className="number-cell">Ordenada</th><th className="number-cell">Recibida</th><th className="number-cell">Previamente facturada</th><th className="number-cell">Actual</th>{canCost && <><th className="number-cell">Costo de orden de compra</th><th className="number-cell">Costo recibido</th></>}<th className="number-cell">Precio factura</th><th>Diferencias</th></tr></thead><tbody>{visibleLines.map(line => <tr key={line.id}><td><strong>{line.purchase_receipt_folio} · #{line.line_number}</strong><small>{line.description}</small></td><td className="number-cell">{line.ordered_quantity}</td><td className="number-cell">{line.received_quantity}</td><td className="number-cell">{line.previously_invoiced}</td><td className="number-cell">{line.quantity}</td>{canCost && <><td className="number-cell">{money(line.order_unit_cost, detail.currency_code)}</td><td className="number-cell">{money(line.received_unit_cost, detail.currency_code)}</td></>}<td className="number-cell">{money(line.invoiced_unit_price, detail.currency_code)}</td><td>{line.differences.length ? <Badge tone="warning">Visible</Badge> : <Badge tone="success">Conciliada</Badge>}</td></tr>)}</tbody></Table>}</PagedCollection> : <PagedCollection items={detail.expense_lines} resetKey={detail.id} label="conceptos fiscales">{visibleLines=><Table><thead><tr><th>Concepto fiscal</th><th>Cantidad / unidad</th><th>Distribución interna</th><th className="number-cell">Subtotal</th><th className="number-cell">Traslados</th><th className="number-cell">Retenciones</th><th className="number-cell">Total</th></tr></thead><tbody>{visibleLines.map(line => <tr key={line.id}><td><strong>{line.product_service_code ?? "Sin clave SAT"}</strong><small>{line.description} · Objeto {line.tax_object_code ?? "—"}</small></td><td>{line.quantity}<small>{line.unit_code ?? "—"} · {line.unit_name ?? "Sin unidad"}</small></td><td>{line.expense_category ?? "Sin categoría"}<small>{[line.cost_center_reference, line.project_reference].filter(Boolean).join(" · ") || "Sin centro/proyecto"}</small></td><td className="number-cell">{money(line.subtotal - line.discount_amount, detail.currency_code)}</td><td className="number-cell">{money(line.tax_amount, detail.currency_code)}</td><td className="number-cell">{money(line.withheld_tax_amount, detail.currency_code)}</td><td className="number-cell"><strong>{money(line.total, detail.currency_code)}</strong></td></tr>)}</tbody></Table>}</PagedCollection>}</> },{ id: "documents", label: `Expediente (${detail.documents.length})`, content: <><section className="invoice-document-dossier"><header><div><h3>Expediente fiscal</h3><p>La validación local compara XML, proveedor, UUID, moneda, fecha y total. El estatus SAT exige evidencia separada.</p></div>{canDocuments && <><input ref={fileInput} hidden type="file" accept=".xml,.pdf,application/xml,text/xml,application/pdf" onChange={event => event.target.files?.[0] && void attachDocument(event.target.files[0])} /><Button size="sm" loading={busy} onClick={() => fileInput.current?.click()}><Upload size={14} /> Adjuntar XML/PDF</Button></>}</header>
        ...
        {canVerifySat && detail.documents.some(item => item.document_role === "cfdi_xml") && <div className="sat-verification-actions"><a href="https://verificacfdi.facturaelectronica.sat.gob.mx/" target="_blank" rel="noreferrer">Verificar en SAT <ExternalLink size={13} /></a><Button size="sm" onClick={() => setAction({ kind: "sat", reason: "", evidence: "", amount: "", series: "", folio: "", satStatus: "valid" })}>Registrar resultado</Button></div>}
      </section></> },{ id: "history", label: "CxP e historial", content: <>{detail.payable && <section className="purchase-order-history"><h3>Cuentas por pagar y ajustes</h3><p>Importe original {money(detail.payable.original_amount, detail.currency_code)} · saldo {money(detail.payable.outstanding_amount, detail.currency_code)} · {conditionLabel(detail.payable.condition)}.</p>{detail.payable.adjustments.map(item => <article key={item.id}><span><strong>{item.adjustment_type === "credit_note" ? "Nota de crédito" : "Reversa"} · {money(item.amount, detail.currency_code)}</strong><small>{new Date(item.occurred_at).toLocaleString("es-MX")}</small></span><p>{item.reason ?? "Sin motivo"}</p></article>)}</section>}<section className="purchase-order-history"><h3>Historial auditado</h3><PagedCollection items={detail.audit} resetKey={detail.id} label="eventos">{visibleAudit=><>{visibleAudit.map(item => <article key={item.id}><span><strong>{auditLabel(item.action)}</strong><small>{new Date(item.created_at).toLocaleString("es-MX")}</small></span></article>)}</>}</PagedCollection></section></> }]} /><div className="purchase-order-actions">{canAuthorize && detail.status === "draft" && detail.differences.length > 0 && !detail.differences_authorized_at && <Button onClick={() => setAction({ kind: "authorize", reason: "", evidence: "", amount: "", series: "", folio: "", satStatus: "valid" })}>Autorizar diferencias</Button>}{canExpense && detail.status === "draft" && detail.source_kind === "expense" && !detail.expense_approved_at && <Button onClick={() => setAction({ kind: "approve_expense", reason: "", evidence: "", amount: "", series: "", folio: "", satStatus: "valid" })}>Aprobar gasto</Button>}{canConfirm && detail.status === "draft" && (detail.source_kind === "receipt" || Boolean(detail.expense_approved_at)) && <Button variant="primary" loading={busy} onClick={() => void confirm()}>Confirmar y crear CxP</Button>}{canCredit && detail.status === "confirmed" && detail.payable && detail.payable.outstanding_amount > 0 && <Button onClick={() => setAction({ kind: "credit", reason: "", evidence: "", amount: "", series: "NC", folio: "", satStatus: "valid" })}>Nota de crédito</Button>}{canReverse && detail.status === "confirmed" && detail.payable?.outstanding_amount === detail.payable?.original_amount && <Button variant="danger" onClick={() => setAction({ kind: "reverse", reason: "", evidence: "", amount: "", series: "", folio: "", satStatus: "valid" })}><RotateCcw size={15} /> Revertir</Button>}</div></div>}</Drawer>
...
function PaymentCalendar({ rows, totals, total, loading, error, view, anchor, page, onPage, onView, onAnchor, onOpen }: {
  ...
  return <section className="payment-calendar-workspace">
    ...
    {view === "table" ? <DataState loading={loading} error={error} hasData={rows.length} empty="No hay CxP con vencimiento en este periodo."><><Table><thead><tr><th>Vencimiento</th><th>Proveedor</th><th>Factura</th><th>Pronto pago</th><th>Estado</th><th className="number-cell">Saldo</th><th className="number-cell">Total estimado</th></tr></thead><tbody>{rows.map(item => <InteractiveTableRow key={item.id} className="payment-calendar-table-row" label={`Abrir cuenta por pagar ${item.invoice_number} de ${item.supplier_name}`} onActivate={() => onOpen(item)}><td>{date(item.due_date)}</td><td><strong>{item.supplier_name}</strong><small>{item.supplier_code}</small></td><td><strong>{item.invoice_number}</strong>{item.payment_reference && <small>Pago {item.payment_reference}</small>}</td><td>{item.eligible_prompt_payment?<><Badge tone="success">{item.eligible_prompt_payment.discount_expression} · {item.eligible_prompt_payment.effective_discount_percent}%</Badge><small>Hasta {date(item.eligible_prompt_payment.deadline)} · ahorro {money(item.eligible_prompt_payment.estimated_savings??0,item.currency_code)} · pagar {money(item.eligible_prompt_payment.estimated_total,item.currency_code)}</small></>:<span>Sin descuento vigente</span>}</td><td><PaymentCalendarBadge item={item} /></td><td className="number-cell"><strong>{money(item.outstanding_amount, item.currency_code)}</strong></td><td className="number-cell"><strong>{money(item.eligible_prompt_payment?.estimated_total??item.outstanding_amount,item.currency_code)}</strong></td></InteractiveTableRow>)}</tbody></Table><Pagination page={page} total={total} setPage={onPage} /></></DataState> : <div className="payment-calendar-shell">
...
function SupplierPaymentsWorkspace({ companyId, permissions, mode, initialPaymentId = null, onInitialPaymentOpened }: { companyId: string; permissions: string[]; mode: "payments" | "accounts"; initialPaymentId?: string | null; onInitialPaymentOpened?: () => void }) {
  ...
  async function confirmPayment() {
    ...
    const paymentDraft = confirmDraft; const fingerprint = JSON.stringify(paymentDraft); setBusy(true);
    ...
    const promptDiscount = Number((data as { prompt_payment_discount_amount?: number }).prompt_payment_discount_amount ?? 0);
    toast({ title: "Pago confirmado", description: promptDiscount > 0 ? `Se pagaron ${money(paymentDraft.proposal.total_proposed, paymentDraft.proposal.currency_code)} y se aplicó ${money(promptDiscount, paymentDraft.proposal.currency_code)} de descuento por pronto pago.` : "Las aplicaciones redujeron las CxP; el pago quedó confirmado y no conciliado.", tone: "success" }); await load(); await openPayment(id);
  }
  ...
  return <section className="supplier-payments-workspace"><header className="supplier-payments-heading"><div><h2>Pagos a proveedores</h2><p>Solo nacen desde propuestas aprobadas y permanecen sin conciliación bancaria.</p></div></header>{canConfirm && <section className="approved-payment-proposals"><header><h3>Por pagar</h3><p>Propuestas autorizadas que aún no se han ejecutado como pago.</p></header><DataState loading={loading} error={null} hasData={approved.length} empty="No hay propuestas aprobadas disponibles."><Table><thead><tr><th>Propuesta</th><th>Proveedor</th><th>CxP</th><th className="number-cell">Importe</th><th></th></tr></thead><tbody>{approved.map(proposal => <tr key={proposal.id}><td><strong>{proposal.id.slice(0, 8).toUpperCase()}</strong><small>{proposal.currency_code}</small></td><td>{proposal.supplier_name}<small>{proposal.supplier_code}</small></td><td>{proposal.line_count}</td><td className="number-cell"><strong>{money(proposal.total_proposed, proposal.currency_code)}</strong></td><td><Button size="sm" loading={busy} onClick={() => void openApprovedProposal(proposal)}>Registrar/ver pago</Button></td></tr>)}</tbody></Table><Pagination page={approvedPage} total={approvedTotal} setPage={setApprovedPage} /></DataState></section>}<div className="supplier-payments-list-heading"><h3>Pagos realizados</h3><p>Pagos ya ejecutados; consulta sus aplicaciones, comprobantes y REP.</p></div><DataToolbar search={query} onSearchChange={value => { setQuery(value); setPage(1); }} placeholder="Proveedor o referencia" results={total} activeFilters={status === "all" ? 0 : 1} onClear={() => setStatus("all")} filters={<Select ariaLabel="Estado del pago" value={status} onValueChange={value => { setStatus(value); setPage(1); }} options={[{ value: "all", label: "Todos" }, { value: "confirmed", label: "Confirmados" }, { value: "reversed", label: "Revertidos" }]} />} /><DataState loading={loading} error={error} hasData={payments.length} empty="No hay pagos con estos filtros."><Table><thead><tr><th>Referencia</th><th>Proveedor</th><th>Fecha efectiva</th><th>Cuenta</th><th>Estado</th><th>REP</th><th>Aplicaciones</th><th className="number-cell">Importe</th></tr></thead><tbody>{payments.map(payment => <InteractiveTableRow key={payment.id} className="purchase-order-row" label={`Abrir pago ${payment.reference}`} onActivate={() => void openPayment(payment.id)}><td><strong>{payment.reference}</strong><small>{payment.payment_method}</small></td><td><strong>{payment.supplier_name}</strong><small>{payment.supplier_code}</small></td><td>{date(payment.effective_date)}</td><td>{payment.account_alias}<small>{payment.bank_name} · {payment.masked_ending}</small></td><td><PaymentBadge status={payment.status} /><small>Sin conciliación</small></td><td><RepBadge status={payment.rep_status} /></td><td>{payment.application_count}</td><td className="number-cell"><strong>{money(payment.total_amount, payment.currency_code)}</strong></td></InteractiveTableRow>)}</tbody></Table><Pagination page={page} total={total} setPage={setPage} /></DataState>
    ...
    <Drawer open={Boolean(paymentDetail)} onOpenChange={open => !open && !busy && setPaymentDetail(null)} title={paymentDetail ? `Pago ${paymentDetail.reference}` : "Pago"} className="purchase-order-detail-drawer payment-proposal-drawer">
      {paymentDetail && <div className="purchase-order-detail"><Card className="purchasing-document-summary"><header><div><PaymentBadge status={paymentDetail.status} /> <Badge tone="warning">No conciliado</Badge> <RepBadge status={paymentDetail.rep_status} /></div><strong>{money(paymentDetail.total_amount, paymentDetail.currency_code)}</strong></header><dl><div><dt>Proveedor</dt><dd>{paymentDetail.supplier.display_name} · {paymentDetail.supplier.code}</dd></div><div><dt>Fecha efectiva</dt><dd>{date(paymentDetail.effective_date)}</dd></div><div><dt>Cuenta pagadora</dt><dd>{paymentDetail.paying_account.alias} · {paymentDetail.paying_account.bank_name} · {paymentDetail.paying_account.masked_ending}</dd></div><div><dt>Forma / referencia</dt><dd>{paymentDetail.payment_method} · {paymentDetail.reference}</dd></div></dl></Card><PurchasingDocumentTabs key={paymentDetail.id} sections={[{ id: "lines", label: `Aplicaciones (${paymentDetail.applications.length})`, content: <><PagedCollection items={paymentDetail.applications} resetKey={paymentDetail.id} label="aplicaciones">{visibleApplications=><Table><thead><tr><th>Factura</th><th>Método</th><th>UUID</th><th className="number-cell">Saldo anterior</th><th className="number-cell">Pago</th><th className="number-cell">Descuento</th><th className="number-cell">Saldo insoluto</th></tr></thead><tbody>{visibleApplications.map(application => { const promptDiscount=Number(application.prompt_payment_discount_amount??0); return <tr key={application.id}><td><strong>{application.invoice_number}</strong><small>{date(application.due_date)}</small></td><td>{application.payment_method_code ?? "Sin clave"}</td><td><small>{application.invoice_uuid ?? "Sin UUID"}</small></td><td className="number-cell">{money(application.balance_before, paymentDetail.currency_code)}</td><td className="number-cell"><strong>{money(application.amount-promptDiscount, paymentDetail.currency_code)}</strong></td><td className="number-cell">{promptDiscount>0 ? money(promptDiscount, paymentDetail.currency_code) : "—"}</td><td className="number-cell">{money(application.balance_after, paymentDetail.currency_code)}</td></tr>; })}</tbody></Table>}</PagedCollection></> },{ id: "documents", label: `Comprobantes (${paymentDetail.documents.length})`, content: <><section className="invoice-document-dossier payment-evidence-dossier"><header><div><h3>Comprobantes y recibo de pago</h3><p>Archivos privados e inmutables. La validación local del XML no sustituye la consulta oficial SAT.</p></div>{canManageDocuments && paymentDetail.status === "confirmed" && <div className="payment-evidence-actions"><input ref={bankReceiptInput} hidden type="file" accept="application/pdf,image/jpeg,image/png" onChange={event => event.target.files?.[0] && void attachPaymentDocument(event.target.files[0], "bank_receipt")} /><input ref={repInput} hidden type="file" accept=".xml,application/xml,text/xml" onChange={event => event.target.files?.[0] && void attachPaymentDocument(event.target.files[0], "rep_xml")} /><Button size="sm" loading={busy} onClick={() => bankReceiptInput.current?.click()}><Upload size={14} /> Comprobante</Button><div style={{ display: "grid", justifyItems: "end", gap: 4 }}><Button size="sm" loading={busy} onClick={() => repInput.current?.click()}><FilePlus2 size={14} /> Adjuntar recibo de pago</Button><small style={{ color: "var(--muted)", fontSize: 11, lineHeight: 1.2, whiteSpace: "nowrap" }}>XML del recibo de pago (REP)</small></div></div>}</header>
        ...
        </section></> },{ id: "history", label: "Historial", content: <>{paymentDetail.reversal_reason && <section className="proposal-decision-reason"><strong>Motivo de reversa</strong><p>{paymentDetail.reversal_reason}</p></section>}<section className="purchase-order-history"><h3>Historial auditado</h3>{paymentDetail.audit.map(item => <article key={item.id}><span><strong>{paymentAuditLabel(item.action)}</strong><small>{new Date(item.created_at).toLocaleString("es-MX")}</small></span></article>)}</section></> }]} /><div className="purchase-order-actions">{canReverse && paymentDetail.status === "confirmed" && <Button variant="danger" onClick={() => setReversalReason("")}><RotateCcw size={15} /> Revertir pago</Button>}</div></div>}
    </Drawer>
