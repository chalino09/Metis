...
const promptPaymentBreakdown = (option: PromptPaymentOption, amount: number, currency: string) => {
  let remaining = amount;
  return option.discount_expression.split("+").map(component => {
    const percent = Number(component.replace("%", ""));
    const saving = remaining * percent / 100;
    remaining -= saving;
    return `${component.replace("%", "")}%: −${money(saving, currency)}`;
  }).join(" · ");
};
