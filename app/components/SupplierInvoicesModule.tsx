...
export function SupplierInvoicesView({ companyId, permissions }: { companyId: string; permissions: string[] }) {
  ...
  return <div className="content-frame supplier-invoice-module">
    ...
    <Drawer open={Boolean(detail)} onOpenChange={open => !open && !busy && setDetail(null)} title={detail ? `Factura ${[detail.series, detail.folio].filter(Boolean).join("-")}` : "Factura"} className="purchase-order-detail-drawer">{detail && <div className="purchase-order-detail supplier-invoice-detail">
      ...
      <dl><div><dt>Proveedor</dt><dd>{detail.supplier.display_name} · {detail.supplier.code}</dd></div><div><dt>Origen</dt><dd>{detail.purchase_order ? `OC ${detail.purchase_order.folio} · ${detail.receipts.map(item => item.folio).join(", ")}` : "Gasto o servicio aprobado sin recepción"}</dd></div><div><dt>Emisión / vencimiento</dt><dd>{date(detail.issued_date)} / {date(detail.due_date)}<small>{detail.due_date_source === "supplier_terms" ? `Calculado con ${detail.supplier_payable_term_days_snapshot ?? 0} días desde emisión` : detail.due_date_source === "manual_override" ? `Excepción auditada: ${detail.due_date_override_reason}` : "Documento anterior a la configuración de plazos"}</small></dd></div><div><dt>Pronto pago</dt><dd>Consulta el beneficio vigente en Pagos → Agenda.<small>La factura conserva las condiciones existentes al crear el borrador.</small></dd></div><div><dt>Moneda / tipo de cambio</dt><dd>{detail.currency_code} · {detail.exchange_rate} → {detail.base_currency_code} ({money(detail.base_total, detail.base_currency_code)})</dd></div><div><dt>CxP</dt><dd>{detail.payable ? `${money(detail.payable.outstanding_amount, detail.currency_code)} · ${conditionLabel(detail.payable.condition)}` : "No generada mientras sea borrador"}</dd></div></dl>
