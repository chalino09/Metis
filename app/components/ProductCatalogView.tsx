...
import { Archive, ChefHat, CookingPot, PackageSearch, Plus, RefreshCw, UtensilsCrossed } from "lucide-react";
...
function RestaurantCatalogTable({role,rows,canManage,onEdit}:{role:RestaurantCatalogRole;rows:ProductListRow[];canManage:boolean;onEdit:(row:ProductListRow)=>void}) {
  ...
  if(role==="preparation")return <div className="table-wrap surface-table"><table className="restaurant-catalog-table"><thead><tr><th>Código</th><th>Base reutilizable</th><th>Categoría</th><th>Rendimiento por tanda</th><th>Receta</th><th>Uso en recetas</th><th>Estado</th></tr></thead><tbody>{rows.map(row=>{const state=recipeState(row);return <InteractiveTableRow key={row.id} disabled={!canManage} className={canManage?"product-catalog__row":undefined} label={`Editar base ${row.name}`} onActivate={()=>onEdit(row)}><td className="mono"><strong>{row.internal_sku}</strong></td><td><strong>{row.name}</strong><small>Base interna</small></td><td>{restaurantCategoryDisplay(row.product_group,role)}</td><td><strong>{row.recipe_yield_quantity?numberFormat(row.recipe_yield_quantity):"—"} {row.recipe_yield_unit_code?restaurantUnitShortLabel(row.recipe_yield_unit_code):""}</strong><small>{row.recipe_status==="active"?"Por tanda":"Se define en la receta"}</small></td><td><Badge tone={state.tone}>{state.label}</Badge><small>{state.detail}</small></td><td><strong>{row.usage_count??0}</strong><small>{(row.usage_count??0)===1?"receta activa":"recetas activas"}</small></td><td><Badge tone={row.is_active?"success":"neutral"}>{row.is_active?"Activa":"Inactiva"}</Badge></td></InteractiveTableRow>;})}</tbody></table></div>;
...
export function ProductCatalogView({ companyId, permissions, experience="core" }: { companyId:string; permissions:string[]; experience?:ProductExperience }) {
  ...
  async function openNew(role:RestaurantCatalogRole=catalogRole){await loadTaxCategories();if(canUseGuidedCreate){setCreationOpen(true);return;}setTaxEditorOpen(false);setCostContext(null);setCostDraft({amount:"",reason:""});setRawPurchaseUnit({code:"",factor:""});setPurchasePresentationSelection(RESTAURANT_NO_PURCHASE_PRESENTATION);setPurchasePresentationOther("");setOtherRestaurantCategory("");setRestaurantCategoryIsOther(false);setDraft({...emptyDraft(),unit:experience==="restaurant"?restaurantUnitCode(null,role==="ingredient"?"g":role==="preparation"?"":"piece"):"",inventoryPolicy:experience==="restaurant"&&role!=="ingredient"?"not_required":"tracked",sellable:experience!=="restaurant"||role==="dish"});}
    ...
    <Drawer open={Boolean(draft)} onOpenChange={open=>{if(!open&&!saving){detailRequestId.current+=1;setDraft(null);setEditingReadiness(null);setDetailLoading(false);setCostContext(null);}}} title={catalogFormTitle} className="product-catalog__drawer">
      {draft&&(detailLoading?<div className="product-catalog__loading" role="status" aria-live="polite" aria-busy="true"><span className="sr-only">Cargando datos del {catalogSingular}…</span><span className="product-catalog__loading-line product-catalog__loading-line--title"/><span className="product-catalog__loading-line"/><span className="product-catalog__loading-line product-catalog__loading-line--short"/><span className="product-catalog__loading-card"/><span className="product-catalog__loading-card"/></div>:<form className={`product-catalog__form ${experience === "restaurant" ? "product-catalog__form--restaurant" : ""} ${draft.id ? "product-catalog__form--editing" : ""}`} onSubmit={saveProduct}>
        ...
        <div className="product-catalog__workspace">
          ...
          {draft.id&&canManageCosts&&costContext&&(experience!=="restaurant"||catalogRole==="ingredient")&&<section className="product-catalog__cost-capture product-catalog__section--wide"><header><div><strong>Valuación de inventario</strong><p>Captura manual para altas o correcciones puntuales. Para muchos insumos, usa la importación masiva de costos.</p></div><Badge tone={costContext.matrix_ready?"success":"warning"}>{costContext.matrix_ready?costMethodLabel(costContext.cost_method):"Configuración de costos pendiente"}</Badge></header>{costContext.matrix_ready?<><div className="product-catalog__cost-summary"><span>Costo vigente<strong>{costContext.current_cost?`${numberFormat(costContext.current_cost.amount)} ${costContext.currency_code}`:"Sin costo"}</strong></span><span>Moneda contable<strong>{costContext.currency_code}</strong></span></div><div className="product-catalog__cost-fields"><Field label="Nuevo costo vigente"><Input inputMode="decimal" value={costDraft.amount} onChange={event=>setCostDraft({...costDraft,amount:event.target.value})} placeholder="0.00"/></Field><Field label="Motivo obligatorio"><Input maxLength={240} value={costDraft.reason} onChange={event=>setCostDraft({...costDraft,reason:event.target.value})} placeholder="Ej. Alta inicial para valuación"/></Field><Button type="button" variant="secondary" loading={savingCost} disabled={!costDraft.reason.trim()||!(Number(costDraft.amount.replace(",","."))>0)} onClick={()=>void saveCurrentCost()}>Guardar costo</Button></div></>:<p className="product-catalog__cost-warning">Completa la configuración de costos para habilitar la captura.</p>}</section>}
          {draft.id&&(catalogRole === "dish"||catalogRole === "preparation")&&canManageRecipes&&<section className="product-catalog__recipe-action product-catalog__section--wide"><div><strong>{catalogRole === "preparation" ? "Receta por tanda" : "Receta por porción"}</strong><p>{catalogRole === "preparation" ? "Administra los insumos y el rendimiento desde un solo editor.":"Administra los insumos y bases que consume una porción."}</p></div><Button type="button" variant="secondary" onClick={()=>{setRecipeProduct({id:draft.id!,name:draft.name,recipeKind:catalogRole==="preparation"?"preparation":"dish"});setDraft(null);}}><ChefHat size={16} aria-hidden="true"/> Editar receta</Button></section>}
