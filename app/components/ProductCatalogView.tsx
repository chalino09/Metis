...
export function ProductCatalogView({ companyId, permissions }: { companyId:string; permissions:string[] }) {
  ...
  async function openEdit(row:ProductListRow){
    ...
    const [{data,error:detailError},costResult]=await Promise.all([
      client.from("products").select("id, internal_sku, alpha_sku, name, barcode, unit, product_group, tax_category_id, is_inventory_tracked, is_sellable, is_active, updated_at").eq("company_id",companyId).eq("id",row.id).single(),
  ...
  async function openNew(){await loadTaxCategories();setCostContext(null);setCostDraft({amount:"",reason:""});setDraft(emptyDraft());}
