"use client";
import { DataToolbar, Button, Drawer, Input, Modal, Select } from "./reui/inventory-controls";

import { Archive, ArrowRight, ChefHat, CookingPot, PackageSearch, Plus, RefreshCw, Truck, UploadCloud, UtensilsCrossed } from "lucide-react";
...
import { DataPagination, DataState, InteractiveTableRow, PageHeading } from "@/app/components/ui/data";
...
function recipeState(row:ProductListRow) {
  ...
  if(row.recipe_status==="active")return {label:"Receta activa",tone:"success" as const,detail:null};
...
function RecipeStateCell({state}:{state:ReturnType<typeof recipeState>}) {
  return <div className="restaurant-catalog-table__recipe-state"><Badge tone={state.tone}>{state.label}</Badge>{state.detail&&<small>{state.detail}</small>}</div>;
}
...
export function ProductCatalogView({ companyId, permissions, experience="core" }: { companyId:string; permissions:string[]; experience?:ProductExperience }) {
  ...
  const canRegisterRestaurantReceipt = experience==="restaurant"&&permissions.includes("view_purchase_receipts")&&permissions.includes("manage_purchase_receipt_drafts")&&permissions.includes("confirm_purchase_receipts");
  ...
  const load = useCallback(async (force=false) => {
    ...
    if(!queryError)queryCache.set(key,next);setRows(next.rows);setTotal(next.total);setError(queryError?catalogLoadError(queryError,catalogPlural):null);setLoading(false);
  ...
  async function saveCurrentCost(productId:string,reason:string){
    if(!costContext)return false;
    const amount=Number(costDraft.amount.replace(",","."));
    if(!Number.isFinite(amount)||amount<=0||!reason.trim())return false;
    setSavingCost(true);
    const {data,error:costError}=await getSupabaseClient().rpc("set_product_current_cost",{p_company_id:companyId,p_product_id:productId,p_amount:amount,p_reason:reason.trim(),p_expected_cost_id:costContext.current_cost?.id??null});
  ...
  async function saveProduct(event:FormEvent){
    ...
    if(saveError)toast({title:`No se pudo guardar el ${catalogSingular}`,description:errorMessage(saveError,"Verifica los datos e intenta nuevamente."),tone:"error"});
      ...
      const manualCost=Number(costDraft.amount.replace(",","."));
      const currentCost=costContext?.current_cost?.amount??null;
      const manualCostChanged=experience==="restaurant"&&catalogRole==="ingredient"&&Boolean(saved?.id)&&Boolean(costContext?.matrix_ready)&&Number.isFinite(manualCost)&&manualCost>0&&(currentCost==null||Math.abs(manualCost-currentCost)>.0000005);
      if(manualCostChanged&&saved?.id){
        const costSaved=await saveCurrentCost(saved.id,normalized.reason);
        if(!costSaved){
          idempotency.clear("save-product");
          setDraft(current=>current?{...current,updatedAt:saved.updated_at??current.updatedAt}:current);
          setSaving(false);
  ...
  const purchasePresentationName=restaurantPurchasePresentationName(purchaseUnit.code||purchasePresentationOther);
  const manualCost=Number(costDraft.amount.replace(",","."));
  const currentManualCost=costContext?.current_cost?.amount??null;
  const manualCostChanged=experience==="restaurant"&&catalogRole==="ingredient"&&Boolean(costContext?.matrix_ready)&&Number.isFinite(manualCost)&&manualCost>0&&(currentManualCost==null||Math.abs(manualCost-currentManualCost)>.0000005);
  const purchaseEntryHref=draft?.id?`/satrapy/compras/recepciones?action=nueva&insumo=${encodeURIComponent(draft.internalSku)}`:"/satrapy/compras/recepciones";
  ...
  useEffect(()=>{
    if(!manualCostChanged)return;
    const warnBeforeUnload=(event:BeforeUnloadEvent)=>event.preventDefault();
    window.addEventListener("beforeunload",warnBeforeUnload);
    return ()=>window.removeEventListener("beforeunload",warnBeforeUnload);
  },[manualCostChanged]);
  function discardProductDraft(){
    if(manualCostChanged&&!window.confirm("Tienes un costo manual sin guardar. Si sales, se perderá esa captura. ¿Quieres descartarla?"))return;
  ...
  return <div className="content-frame product-catalog">
    ...
    <Drawer open={Boolean(draft)} onOpenChange={open=>{if(!open&&!saving&&!savingDishPrice)discardProductDraft();}} title={catalogFormTitle} className="product-catalog__drawer">
        ...
        <div className="product-catalog__workspace">
          ...
          <section className="product-catalog__section product-catalog__inventory">
            ...
            {experience === "restaurant" ? catalogRole === "ingredient" ? <>
              ...
              <div className="product-catalog__ingredient-flow" aria-label="Dónde capturar cada cantidad">
                <div><span>1</span><p><strong>Aquí defines la presentación</strong><small>{purchaseUnit.code?`Se compra por ${purchasePresentationName} y se controla en ${restaurantUnitShortLabel(draft.unit)}.`:"Selecciona cómo llega este insumo del proveedor."}</small></p></div>
                <div><span>2</span><p><strong>En Entradas capturas cantidad y precio</strong><small>Ejemplo: 3 litros recibidos a $40 por litro.</small></p></div>
                <div><span>3</span><p><strong>En cada receta capturas el consumo</strong><small>Ejemplo: 30 ml de aceite por platillo.</small></p></div>
              </div>
              {draft.id&&canRegisterRestaurantReceipt&&<Link className="product-catalog__receipt-link ui-button ui-button--secondary ui-button--sm" href={purchaseEntryHref} onClick={event=>{if(manualCostChanged&&!window.confirm("Tienes un costo manual sin guardar. Si continúas a Entradas, se perderá esa captura. ¿Quieres descartarla?"))event.preventDefault();}}><Truck size={16} aria-hidden="true"/> Registrar entrada de este insumo <ArrowRight size={15} aria-hidden="true"/></Link>}
