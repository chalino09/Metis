...
export async function processWhatsappText(admin:AdminClient,input:{companyId:string;connectionId:string;locationId:string;customerId?:string|null;messageId:string;sender:string;message:string;rawPayload:string}){
  ...
  const registered=await admin.rpc("register_integration_webhook",{p_company_id:input.companyId,p_provider_code:"meta_whatsapp",p_provider_event_id:input.messageId,p_event_type:"messages",p_payload_sha256:digest});
  if(registered.error)throw new Error(registered.error.message);
  const receipt=registered.data as {receipt_id?:string;duplicate?:boolean};
  if(!receipt.receipt_id)throw new Error("No se registró el webhook.");
  if(receipt.duplicate)return{duplicate:true,receiptId:receipt.receipt_id,requestId:null,status:"duplicate"};
